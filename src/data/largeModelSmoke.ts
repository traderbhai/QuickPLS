...
export type LargeModelSmokeProfile = "applied" | "stress";

/** Deterministic interaction fixture; loaded only by the visual acceptance query. */
export function largeModelSmokeProject(profile: LargeModelSmokeProfile = "applied"): {
  dataset: Dataset;
  nodes: Array<Node<ConstructData>>;
  edges: Edge[];
} {
  const constructCount = profile === "stress" ? 100 : 20;
  const indicatorCount = profile === "stress" ? 300 : 80;
  const indicatorsPerConstruct = indicatorCount / constructCount;
  const columns = Array.from({ length: indicatorCount }, (_, index) => {
  ...
  const nodes: Array<Node<ConstructData>> = Array.from({ length: constructCount }, (_, index) => {
    ...
    const shortName = `C${String(number).padStart(2, "0")}`;
    return {
      id: `construct-${number}`,
      type: "construct",
      ...
      data: {
        ...
        shortName,
        mode: "reflective",
        indicators: columns.filter((column) => column.startsWith(`${shortName}_`)),
      },
    };
  });
  ...
  return {
    dataset: { id: `${profile}-model-smoke`, name: `${profile === "stress" ? "Stress" : "Large"} model acceptance fixture`, kind: "raw", columns, rows, missing: 0, rowCount: rows.length, sampleSize: rows.length, fingerprint: `quickpls-${profile}-model-smoke-v1` },
    nodes,
    edges,
  };
}
