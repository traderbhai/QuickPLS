...
export function ReportsWorkspace() {
  ...
  const readyOutputs = [!tableExportDisabledReason, !tableExportDisabledReason, !xlsxDisabledReason, !pdfDisabledReason, !svgDisabledReason].filter(Boolean).length;
  ...
  return <WorkspacePage className="publication-workspace report-v2-workspace report-v213-workspace report-v2100-workspace report-v219-workspace report-v2310-workspace" data-report-export-flow="v2.31" data-v219-mockup-screen="report" data-v2310-report-wizard="true">
    <PageHeader title="Publication report" description="Assemble figure, tables, provenance, and optional interpretation notes from one validated run." actions={<StatusBadge status={selectedRun ? "validated" : "warning"}>{selectedRun ? "run selected" : "model-only"}</StatusBadge>} />
    ...
    <Panel title="Report package" description="Selected preset, run, tables, figure, and export readiness." className="report-v2-hero report-v213-hero">
      <div className="report-v2-hero-copy">
        ...
        <p>{selectedRun ? `${selectedRun.name} is selected with ${tables.length} result table(s), ${readyOutputs} ready export output(s), and ${publicationDiagramSettings.precision}-decimal figure labels.` : "Select or run an analysis to unlock result tables and full report exports."}</p>
      </div>
      <div className="report-v2-hero-metrics" aria-label="Report export readiness">
        ...
        <MetricCard label="Figure" value={svgDisabledReason ? "Model only" : "SVG ready"} detail={publicationDiagramSettings.layoutSource.replace("_", " ")} tone={svgDisabledReason ? "warning" : "success"} />
        <MetricCard label="Outputs" value={`${readyOutputs}/5`} detail="CSV, HTML, XLSX, PDF path, SVG" />
      </div>
    </Panel>
    ...
    <Panel title="Choose export preset" description="Presets only change presentation defaults; result values remain unchanged." actions={<StatusBadge status={includeInterpretationNotes ? "info" : "validated"}>{includeInterpretationNotes ? "interpretation included" : "numeric by default"}</StatusBadge>} className="report-v2-command-center report-v213-command-center">
      ...
      <ol className="export-stepper report-stepper" aria-label="Publication export steps">
        ...
        <li className={diagramSvg && tables.length ? "complete" : "active"}><b>3</b><span>Review figure and tables</span></li>
