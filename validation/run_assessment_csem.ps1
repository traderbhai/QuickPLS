...
& $RscriptPath --vanilla `
  (Join-Path $root "validation\assessment_csem_reference.R") `
  (Join-Path $root "validation\fixtures\corporate_reputation.csv") `
  (Join-Path $root "validation\results\assessment_csem_0_6_1.csv")

python validation\assessment_csem_compare.py
