...
if ([string]::IsNullOrWhiteSpace($ExportPath)) {
    ...
    $ExportPath = Join-Path $repositoryRoot "validation\results\v247-native-prediction-$stamp.xlsx"
}
...
$env:QUICKPLS_ACCEPTANCE_SCOPE = "prediction"
$env:QUICKPLS_PREDICTION_NATIVE_EXPORT_PATH = $ExportPath
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Focused packaged PLSpredict/CVPAT acceptance failed with exit code $LASTEXITCODE."
    }
    ...
    Remove-Item Env:QUICKPLS_ACCEPTANCE_SCOPE,Env:QUICKPLS_PREDICTION_NATIVE_EXPORT_PATH -ErrorAction SilentlyContinue
}
