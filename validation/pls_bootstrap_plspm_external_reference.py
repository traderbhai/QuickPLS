#!/usr/bin/env python3
"""PLS bootstrap matched-resample evidence against python-plspm.

This is development-only validation evidence. It must be run with the
validation virtualenv and is never a QuickPLS runtime dependency.
"""
...
VARIANTS = {
    ...
    "MODE_B": {
        "recipe": ROOT / "validation" / "fixtures" / "simple_reflective.mode_b.recipe.json",
        "mode": Mode.B,
        "scheme": Scheme.PATH,
    },
    "FACTOR": {
        "recipe": ROOT / "validation" / "fixtures" / "simple_reflective.factor.recipe.json",
        "mode": Mode.A,
        "scheme": Scheme.FACTORIAL,
    },
}
RESULTS = ROOT / "validation" / "results"
WORK = RESULTS / "pls_bootstrap_plspm_external_reference"
OUTPUT = RESULTS / "pls_bootstrap_plspm_external_reference.json"
TOLERANCE = 1.0e-6
...
def write_recipe(template: Path, fingerprint: str, variant: str, output: Path) -> None:
    ...
    recipe["id"] = f"00000000-0000-0000-0000-{list(VARIANTS).index(variant) + 601:012d}"
    recipe["metadata"] = {
        "fixture": "PLS bootstrap python-plspm matched-resample validation",
...
def run_plspm(variant: str, data_path: Path) -> dict[tuple[str, str, str, str, str], float]:
    ...
    config.add_lv("x", variant_spec["mode"], MV("x1"), MV("x2"))
    config.add_lv("y", variant_spec["mode"], MV("y1"), MV("y2"))
    model = Plspm(
        data,
        config,
        variant_spec["scheme"],
    ...
    values: dict[tuple[str, str, str, str], float] = {
        ("path", "x", "y", ""): float(paths.loc["y", "x"])
    }
    for index, row in outer.iterrows():
        indicator = str(index)
        construct = "x" if indicator in {"x1", "x2"} else "y"
        values[("loading", construct, "", indicator)] = float(row["loading"])
    return {(variant, *key): values[key] for key in PARAMETERS}
...
def main() -> int:
    ...
    plspm_values = {key: [] for key in parameter_keys}
    accepted = []
    skipped = []
    ...
    for candidate_index in range(MAX_CANDIDATES):
        ...
        try:
            ...
            for variant, spec in VARIANTS.items():
                ...
                write_recipe(spec["recipe"], fingerprint, variant, recipe_path)
                quick.update(run_quickpls(variant, data_path, recipe_path, quick_path))
                plspm.update(run_plspm(variant, data_path))
        ...
        for key in parameter_keys:
            difference = quick[key] - plspm[key]
            row = {
                "variant": key[0],
                "kind": key[1],
                "source": key[2],
        ...
        for key in parameter_keys:
            ...
            plspm_values[key].append(plspm[key])
        accepted.append(
            {
                "candidate": candidate_index,
                "indices": indices,
    ...
    plspm_summary = summarize(plspm_values) if len(accepted) >= 2 else []
    plspm_summary_by_key = {key_from_row(row): row for row in plspm_summary}
    summary_comparisons = []
    for quick_row in quick_summary:
        key = key_from_row(quick_row)
        reference_row = plspm_summary_by_key[key]
        metric_diffs = {
            metric: float(quick_row[metric]) - float(reference_row[metric])
            for metric in [
                "mean",
    ...
    report = {
        "schema_version": 1,
        "kind": "pls_bootstrap_plspm_external_reference_v1",
        "passed": passed,
        "tolerance": TOLERANCE,
        ...
        "fixture": {
            ...
            "recipes": {
                variant: relative(spec["recipe"]) for variant, spec in VARIANTS.items()
            },
            "resample_seed": SEED,
        ...
        "note": "Fixed resample-index PLS integration fixture against python-plspm across path Mode A, Mode B, and factorial variants. This verifies estimator and aggregate bootstrap summary parity on matched resamples; it is not stochastic coverage qualification.",
    }
