...
Tracked manifests remain Labs/absent templates. Live cell reports and manifests
are written only beneath the external campaign root. Source hashes bind clean
...
def assert_runtime_promotion_mechanism() -> None:
    ...
    if (
        ...
        raise ManifestError(
            "runtime promotion unavailable: native execution/export does not consume the embedded exact-cell receipt"
        )
...
def verify(
    ...
    if (
        ...
        or manifest_set.get("surface") != "labs"
        or manifest_set.get("promotion_allowed") is not False
    ...
    for row in manifest_rows:
        ...
        for profile in manifest.get("profile_matrix", []):
            if profile.get("coverage_state") != "release_qualified" or profile.get("evidence_state") != "release_qualified":
                raise ManifestError(f"profile is not release-qualified: {profile.get('profile_id')}")
            for cell in profile.get("procedure_cells", []):
                observed_cells.append(f"{profile.get('profile_id')}::{cell.get('procedure_id')}")
                ...
                if (
                    ...
                    raise ManifestError(
                        f"procedure evidence is missing or stale: {profile.get('profile_id')}::{cell.get('procedure_id')}"
                    )
