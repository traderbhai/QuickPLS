...
if ([string]::IsNullOrWhiteSpace($ExportPath)) {
    ...
    $ExportPath = Join-Path $repositoryRoot "validation\results\v247-native-cbsem-$stamp.xlsx"
}
...
$env:QUICKPLS_ACCEPTANCE_SCOPE = "cbsem"
$env:QUICKPLS_CBSEM_NATIVE_EXPORT_PATH = $ExportPath
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Focused packaged CB-SEM acceptance failed with exit code $LASTEXITCODE."
    }
    ...
    Remove-Item Env:QUICKPLS_ACCEPTANCE_SCOPE,Env:QUICKPLS_CBSEM_NATIVE_EXPORT_PATH -ErrorAction SilentlyContinue
}
