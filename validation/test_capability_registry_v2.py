...
class CapabilityRegistryV2Tests(unittest.TestCase):
    ...
    def test_authoritative_baseline_and_local_references_pass(self):
        ...
        self.assertEqual(report["qualification_link_count"], 59)
        self.assertEqual(report["option_cell_count"], 59)
        self.assertEqual(
            report["option_cell_coverage_counts"],
            {"full": 0, "partial": 46, "absent": 11, "intentionally_excluded": 2},
        )
        self.assertEqual(
        ...
        self.assertEqual(
            report["option_cell_evidence_counts"],
            {
                ...
                "archive_qualified": 1,
                "native_qualified": 0,
                "release_qualified": 41,
            },
        )
        self.assertTrue(report["manifest_evidence_check"]["passed"])
        self.assertEqual(report["manifest_evidence_check"]["mapped_cell_count"], 55)
        self.assertEqual(report["manifest_evidence_check"]["unique_manifest_count"], 49)
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            {"standard": 41, "labs": 16, "legacy": 2, "internal": 0},
        )

    def test_general_sem_two_way_moderated_mediation_bootstrap_is_exact_standard_cell(self):
    ...
    def test_source_qualification_specs_are_linked_exactly_at_their_declared_surfaces(self):
        ...
        for capability_id, cell_id, version, coverage, evidence, surface, reference in cases:
            with self.subTest(cell_id=cell_id):
                ...
                if "evidence_contract" not in specification:
                    self.assertEqual(
                        specification["qualification"]["declared_state"], evidence
                    )
                    continue
