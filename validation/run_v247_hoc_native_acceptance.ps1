...
$reportPath = [System.IO.Path]::GetFullPath((Join-Path $repositoryRoot "validation\results\v247_tauri_native_acceptance_hoc.json"))
$cdpEndpoint = "http://127.0.0.1:9222"
$supervisorStartedUtc = [DateTime]::UtcNow

if ([string]::IsNullOrWhiteSpace($ExportPath)) {
    $stamp = Get-Date -Format "yyyyMMdd-HHmmssfff"
    $ExportPath = Join-Path $repositoryRoot "validation\results\v247-native-hoc-$stamp.xlsx"
}
$ExportPath = [System.IO.Path]::GetFullPath($ExportPath)
if ([string]::IsNullOrWhiteSpace($ReceiptPath)) {
    $ReceiptPath = Join-Path $repositoryRoot "validation\results\v247_hoc_scoped_native_acceptance_receipt_v1.json"
}
...
foreach ($requiredFile in @($desktopExecutable, $harnessPath, $closeHelperPath)) {
    if (-not (Test-Path -LiteralPath $requiredFile -PathType Leaf)) {
        throw "Required focused HOC acceptance input is missing: $requiredFile"
    }
}
if (Test-Path -LiteralPath $ReceiptPath) {
    throw "Refusing to overwrite the append-only focused HOC receipt: $ReceiptPath"
}
...
if (Test-CdpReady) {
    throw "Focused HOC acceptance requires the dedicated loopback CDP endpoint to be closed before launch."
}
...
$env:QUICKPLS_ACCEPTANCE_SCOPE = "hoc"
$env:QUICKPLS_HOC_NATIVE_EXPORT_PATH = $ExportPath
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Focused packaged HOC acceptance failed with exit code $LASTEXITCODE."
    }
    ...
    Remove-Item Env:QUICKPLS_ACCEPTANCE_SCOPE,Env:QUICKPLS_HOC_NATIVE_EXPORT_PATH -ErrorAction SilentlyContinue
}

if (-not $cleanupVerified) {
    throw "Focused HOC acceptance did not close its exact process tree and loopback CDP endpoint cleanly."
}
if (-not (Test-Path -LiteralPath $reportPath -PathType Leaf)) {
    throw "Focused HOC acceptance did not publish its scoped report."
}
$report = Get-Content -LiteralPath $reportPath -Raw -Encoding UTF8 | ConvertFrom-Json
$checkNames = @($report.checks.PSObject.Properties | ForEach-Object { [string]$_.Name })
$offline = $report.checks.hocFunctionalOffline
if ($report.passed -ne $true -or @($report.failures).Count -ne 0 -or @($report.consoleErrors).Count -ne 0 `
    -or [string]$report.runtime -ne "tauri-webview2-cdp" -or [string]$report.focusedRun.scope -ne "hoc" `
    -or $checkNames.Count -ne 17 -or @($checkNames | Sort-Object -Unique).Count -ne 17 `
    ...
    throw "Focused HOC scoped report is incomplete, impure, or not offline within the observed browser/app request boundary."
}
if (-not (Test-Path -LiteralPath $ExportPath -PathType Leaf)) {
    throw "Focused HOC XLSX export is missing: $ExportPath"
}
$projectPath = [string]$report.checks.hocFixtureProvisioning.project
if (-not (Test-Path -LiteralPath $projectPath -PathType Leaf)) {
    throw "Focused HOC project archive is missing: $projectPath"
}
$screenshots = @($report.screenshots | ForEach-Object { Get-ArtifactDescriptor -Path ([string]$_) })
if ($screenshots.Count -ne 11) {
    throw "Focused HOC report must bind exactly 11 screenshot artifacts; found $($screenshots.Count)."
}

$receipt = [pscustomobject]@{
    schema_version = 1
    kind = "quickpls_v247_hoc_scoped_native_acceptance_receipt"
    passed = $true
