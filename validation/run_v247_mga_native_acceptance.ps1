...
if ([string]::IsNullOrWhiteSpace($ExportPath)) {
    ...
    $ExportPath = Join-Path $repositoryRoot "validation\results\v247-native-mga-$stamp.xlsx"
}
...
$env:QUICKPLS_ACCEPTANCE_SCOPE = "mga"
$env:QUICKPLS_MGA_NATIVE_EXPORT_PATH = $ExportPath
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Focused packaged MICOM/MGA acceptance failed with exit code $LASTEXITCODE."
    }
