"""Shared, method-scoped evidence utilities for the PCA v1 promotion factory.

This module deliberately knows only the frozen PCA v1 manifest.  It creates
strict identity reports, binds their claims to exact repository bytes, and
provides a small CLI runner used by the scientific and boundary gates.
"""
...
MANIFEST_PATH = VALIDATION / "methods" / "pca_v1.manifest.json"
REPORT_ROOT = VALIDATION / "results" / "method_factory" / "pca_v1"
WORK_ROOT = REPORT_ROOT / "work"
CLI = ROOT / "target" / "debug" / "qpls.exe"
COMMON_SOURCE = "validation/pca_v1_factory_common.py"
FOCUSED_FACTORY_TEST = "validation/test_pca_v1_factory_evidence.py"
...
def source_descriptors(paths: Iterable[str]) -> list[dict[str, Any]]:
    ...
    for relative in sorted(set(paths)):
        ...
        path = ROOT / relative
        if not path.is_file():
            raise FileNotFoundError(f"required PCA v1 evidence source is missing: {relative}")
        descriptors.append(
            {
                "path": relative,
...
def manifest() -> dict[str, Any]:
    ...
    for key, value in expected.items():
        if feature.get(key) != value:
            raise ValueError(
                f"PCA v1 manifest identity mismatch for {key}: "
...
def ensure_cli() -> dict[str, Any]:
    ...
    completed, execution = run_command(["cargo", "build", "-p", "qpls-cli"], timeout=900)
...
def write_csv(path: Path, variables: Sequence[str], rows: Sequence[Sequence[float | None]]) -> None:
...
def _fingerprint(csv_path: Path, name: str) -> tuple[str, dict[str, Any]]:
    ...
    if completed.returncode != 0:
        raise RuntimeError(f"PCA fixture import failed for {name}: {execution}")
    inspected, inspect_execution = run_command(
        [str(CLI), "inspect", repository_path(project), "--json"]
    )
    if inspected.returncode != 0:
        raise RuntimeError(f"PCA fixture inspect failed for {name}: {inspect_execution}")
...
def run_pca(
    *,
    ...
    retention: dict[str, Any] = {"rule": rule}
    if components is not None:
        retention["components"] = components
    if threshold is not None:
        retention["threshold"] = threshold
    recipe_payload = {
        "schema_version": 3,
        "id": str(uuid.uuid5(uuid.NAMESPACE_URL, f"quickpls:pca-v1:{name}")),
        "created_at": "2026-08-13T00:00:00Z",
        "dataset_fingerprint": fingerprint,
        "model": {
            "id": str(uuid.uuid5(uuid.NAMESPACE_URL, f"quickpls:pca-v1:model:{name}")),
            "name": "PCA v1 factory fixture",
            "constructs": [],
            "paths": [],
            "controls": [],
        ...
        "metadata": {"status": "validated_pca_v1_bounded_scope"},
    }
    recipe.write_text(
    ...
    if expect_success and completed.returncode != 0:
        raise RuntimeError(f"PCA execution failed for {name}: {execution}")
    ...
    identity_passed = (
        ...
        and estimation["method_version"] == "pca_v1"
        and pca["method_version"] == "pca_v1"
        and document["provenance"]["method"] == "pca"
        and document["provenance"]["method_version"] == "pca_v1"
...
def analytic_payload(result: dict[str, Any]) -> dict[str, Any]:
    """Return only the deterministic PCA payload, excluding run metadata."""

    return result["payload"]["estimation"]["pca"]
