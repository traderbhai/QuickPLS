...
if ([string]::IsNullOrWhiteSpace($ExportPath)) {
    ...
    $ExportPath = Join-Path $repositoryRoot "validation\results\v247-native-ols-$stamp.xlsx"
}
...
$env:QUICKPLS_ACCEPTANCE_SCOPE = "ols"
$env:QUICKPLS_OLS_NATIVE_EXPORT_PATH = $ExportPath
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Focused packaged ordinary least squares acceptance failed with exit code $LASTEXITCODE."
    }
    ...
    Remove-Item Env:QUICKPLS_ACCEPTANCE_SCOPE,Env:QUICKPLS_OLS_NATIVE_EXPORT_PATH -ErrorAction SilentlyContinue
}
