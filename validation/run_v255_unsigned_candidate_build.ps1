...
param(
    ...
    [string]$Label = "v2_55_5_oi_sample_catalog"
)
...
if ((Get-Content -LiteralPath (Join-Path $root "package.json") -Raw -Encoding UTF8 | ConvertFrom-Json).version -ne "2.55.5") {
    throw "The 2.55.5 candidate build may run only after all version authorities are 2.55.5."
}
...
foreach ($requiredOutput in @(
    ...
    (Join-Path $releaseDir "bundle\nsis\QuickPLS_2.55.5_x64-setup.exe")
)) {
...
if ($releaseReport.passed -ne $true -or $releaseReport.schema_version -ne 3 -or $releaseReport.version -ne "2.55.5" -or $releaseReport.source.commit -ne $source.commit) {
