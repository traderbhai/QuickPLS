...
$reportPath = [System.IO.Path]::GetFullPath((Join-Path $repositoryRoot "validation\results\v247_tauri_native_acceptance_nca.json"))
$cdpEndpoint = "http://127.0.0.1:9222"
$supervisorStartedUtc = [DateTime]::UtcNow

if ([string]::IsNullOrWhiteSpace($ExportPath)) {
    $stamp = Get-Date -Format "yyyyMMdd-HHmmssfff"
    $ExportPath = Join-Path $repositoryRoot "validation\results\v247-native-nca-$stamp.xlsx"
}
$ExportPath = [System.IO.Path]::GetFullPath($ExportPath)
if ([string]::IsNullOrWhiteSpace($ReceiptPath)) {
    $ReceiptPath = Join-Path $repositoryRoot "validation\results\v247_nca_scoped_native_acceptance_receipt_v2.json"
}
...
foreach ($requiredFile in @($desktopExecutable, $harnessPath, $closeHelperPath)) {
    if (-not (Test-Path -LiteralPath $requiredFile -PathType Leaf)) {
        throw "Required focused NCA acceptance input is missing: $requiredFile"
    }
}
if (Test-Path -LiteralPath $ReceiptPath) {
    throw "Refusing to overwrite the append-only focused NCA receipt: $ReceiptPath"
}
...
if (Test-CdpReady) {
    throw "Focused NCA acceptance requires the dedicated loopback CDP endpoint to be closed before launch."
}
...
try {
    ...
    Remove-Item Env:QUICKPLS_ACCEPTANCE_SCOPE,Env:QUICKPLS_NCA_NATIVE_EXPORT_PATH -ErrorAction SilentlyContinue
}

if (-not $cleanupVerified) {
    throw "Focused NCA acceptance did not close its exact process tree and loopback CDP endpoint cleanly."
}
if (-not (Test-Path -LiteralPath $reportPath -PathType Leaf)) {
    throw "Focused NCA acceptance did not publish its scoped report."
}
$report = Get-Content -LiteralPath $reportPath -Raw -Encoding UTF8 | ConvertFrom-Json
$checkRows = @($report.checks.PSObject.Properties)
$checkNames = @($checkRows | ForEach-Object { [string]$_.Name })
$offline = $report.checks.ncaFunctionalOffline
if ($report.passed -ne $true -or @($report.failures).Count -ne 0 -or @($report.consoleErrors).Count -ne 0 `
    -or [string]$report.runtime -ne "tauri-webview2-cdp" -or [string]$report.focusedRun.scope -ne "nca" `
    -or $checkNames.Count -ne 14 -or @($checkNames | Sort-Object -Unique).Count -ne 14 `
    ...
    throw "Focused NCA scoped report is incomplete, impure, or not offline within the observed browser/app request boundary."
}
if (-not (Test-Path -LiteralPath $ExportPath -PathType Leaf)) {
    throw "Focused NCA XLSX export is missing: $ExportPath"
}
$projectPath = [string]$report.checks.ncaFixtureProvisioning.project
if (-not (Test-Path -LiteralPath $projectPath -PathType Leaf)) {
    throw "Focused NCA project archive is missing: $projectPath"
}
$screenshots = @($report.screenshots | ForEach-Object { Get-ArtifactDescriptor -Path ([string]$_) })
if ($screenshots.Count -ne 11) {
    throw "Focused NCA report must bind exactly 11 screenshot artifacts; found $($screenshots.Count)."
}
