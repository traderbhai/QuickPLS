...
if ([string]::IsNullOrWhiteSpace($ExportPath)) {
    ...
    $ExportPath = Join-Path $repositoryRoot "validation\results\v247-native-logistic-$stamp.xlsx"
}
...
$env:QUICKPLS_ACCEPTANCE_SCOPE = "logistic"
$env:QUICKPLS_LOGISTIC_NATIVE_EXPORT_PATH = $ExportPath
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Focused packaged binary logistic regression acceptance failed with exit code $LASTEXITCODE."
    }

    Get-Item -LiteralPath $ExportPath | Select-Object FullName, Length, LastWriteTime
    Get-Item -LiteralPath (Join-Path $repositoryRoot "validation\results\logistic_v2_packaged_acceptance.json") |
    ...
    Remove-Item Env:QUICKPLS_ACCEPTANCE_SCOPE,Env:QUICKPLS_LOGISTIC_NATIVE_EXPORT_PATH -ErrorAction SilentlyContinue
}
