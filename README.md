...
Current source version: **2.55.4**. This source release centers measurement loading and weight badges on their exact construct–indicator connector paths across Straight, Curved, Orthogonal, and Polyline routing in Model, Results, and Publication diagrams. The correction is presentation-only. It retains the editable measurement-connector routing added in 2.55.3, the 2.55.2 Results → Edit Model unlock fix, the full Corporate Reputation SmartPLS-comparison project introduced in 2.55.1, and now adds the 305-case **Organizational Identification Model** as the fourth bundled sample with a completed result and 27/27 three-decimal screenshot parity. The formal 2.55.0 diagnostic records below remain historical source evidence and are not packaged validation for 2.55.4. Formal first diagnostic `20260822T142953Z` at source `2e3a23f` executed all 14 steps and passed 13; the sole failure was `frontend_typecheck`, where `src/data/v255NamedSemEvidenceFixtures.test.ts` reported TypeScript error `TS2339`. The final consolidated diagnostic `20260823T030939Z` at source `e5723df08b7205ce75f1887c5f4709f235ad893c` passed 14/14, including 453/453 Vitest suites, 1724/1724 tests, all 17 rebaseline assertions, and zero captured console errors. Its report is `validation/results/v255_consolidated_diagnostics_20260823T030939Z/v255_consolidated_diagnostics.json`, has SHA-256 `03da7a8e0db2924d0157eb0cb0ca92e841fffd61f470d5cd16ccd58f87fe9b2a`, and is retained in evidence commit `8a727262c07dd38bae38d8154e1662c78fbb8ee7`. Both formal records use runner SHA-256 `64969b4eb89c9789e586c372532d89b082766a44661cc4feea66b6cf3a0f9796`; they are separate evidence records and are not byte-identical. Renderer console/page-error evidence fails closed, attach-only phases use fresh wrapper-owned processes, and candidate/phase/trusted-driver PID, role, suite, and SHA-256 bindings are closed. This is source evidence, not packaged validation. The superseded final diagnostic `20260823T000930Z`, candidate/install/smoke attempts `20260823T004848Z` and `20260823T005212Z`, portable probe `20260822T233111Z`, all prior candidate attempts, and the earlier 2.55.4 installer built before the fourth sample are historical and ineligible. A fresh 2.55.4 publication candidate still requires its own provenance-bound build, isolated install, installed-and-portable smoke, evidence bundle, final audit, and publication. Latest published public pre-release: [`v2.54.0`](https://github.com/traderbhai/QuickPLS/releases/tag/v2.54.0). Code signing is excluded; method labels such as **Standard** describe bounded analytical capability, not installer signing state.
...
`20260822T233111Z`, and all prior candidate, install, smoke, diagnostic, or probe
attempts are historical and ineligible.
...
- [QuickPLS 2.55.4 Release Notes — measurement label alignment and fourth bundled sample](docs/RELEASE_NOTES_V2_55_4.md)
- [QuickPLS 2.55.3 Release Notes — measurement connector routing](docs/RELEASE_NOTES_V2_55_3.md)
- [QuickPLS 2.55.2 Release Notes — Edit Model state fix](docs/RELEASE_NOTES_V2_55_2.md)
- [QuickPLS 2.55.1 Release Notes — Corporate Reputation sample patch](docs/RELEASE_NOTES_V2_55_1.md)
...
2. From the Launcher, create a project, open a saved project, or open a bundled sample. Corporate Reputation includes the prepared 344-case, 8-construct mixed formative-reflective model. Organizational Identification Model includes all 305 supplied cases, four reflective constructs, 21 modeled indicators, three paths, and a completed PLS-SEM result.
...
Local versioned builds write setup, portable, CLI, and checksum files to `target/release/artifacts/`. The superseded final diagnostic `20260823T000930Z`, candidate/install/smoke attempts `20260823T004848Z` and `20260823T005212Z`, portable probe `20260822T233111Z`, and all prior local Version 2.55.0 candidate, install, smoke, diagnostic, or probe attempts are historical and ineligible. The marker-only install comparison is corrected but has no eligible packaged proof. One new provenance-bound unsigned candidate must still be built and complete the required isolated install plus full installed-and-portable smoke before evidence collection, bundling, final audit, and publication. The latest published files remain the verified Version 2.54.0 setup, portable app, CLI, and checksums on the [`v2.54.0` release page](https://github.com/traderbhai/QuickPLS/releases/tag/v2.54.0).
