...
Status: post-hoc-corrected, waiver-qualified source gate complete; fresh unsigned candidate, installed/portable evidence, evidence bundle, merge/tag, and publication pending.
...
The post-hoc-corrected consolidated gate passed all 14 steps in both the formal first run and the final rerun using the same diagnostic script and suite contract:
...
The qualifying first report is stored under `validation/results/v255_consolidated_diagnostics_20260822T064330Z/` and is bound to source `0ae22eb`. The final report is stored under `validation/results/v255_consolidated_diagnostics_20260822T064832Z/` and is bound to source `0238e20`. Both passed 14/14 under the same script and suite contract.
...
Every earlier 2.55 candidate, including `20260822T052553Z`, is historical and ineligible because the post-hoc reopen-label product correction and the current formal validation postdate it. That correction prioritizes exact typed post-hoc authority over the generic PLS-SEM bootstrap fallback and passed its focused identity suite 17/17, but has not yet been exercised in a packaged candidate. The separate install-comparison correction permits only Tauri's exact three-byte `UNK` → `NSS` package marker transition and rejects every other byte difference. A fresh clean-source, provenance-bound unsigned setup, portable, CLI, and checksum package must be built from the current 2.55 authorities. No fresh candidate, installation, installed or portable smoke, bundle, final audit, merge, tag, or publication success is claimed yet.
