...
Status: waiver-qualified source gate complete; fresh unsigned candidate, installed/portable evidence, evidence bundle, merge/tag, and publication pending.
...
The waiver-qualified consolidated gate passed all 14 steps in both the formal first run and the final rerun using the same diagnostic script and suite contract:
...
The qualifying first report is stored under `validation/results/v255_consolidated_diagnostics_20260822T051258Z/`; the same-script final report is stored under `validation/results/v255_consolidated_diagnostics_20260822T051715Z/`. Both passed 14/14.
...
The earlier 2.55 candidate/install attempt is historical only. Its candidate build was valid, but the install receipt failed because the wrapper wrongly required the installed executable to be byte-identical. The corrected, focused-tested contract permits only Tauri's exact three-byte `UNK` → `NSS` package marker transition and rejects every other byte difference. A fresh clean-source, provenance-bound unsigned setup, portable, CLI, and checksum package must be built from the current 2.55 authorities. No fresh candidate, installation, smoke, bundle, final audit, merge, tag, or publication success is claimed yet.
