...
`Launcher → Data → Canvas → Calculate → Results`. Both waiver-qualified
consolidated source-gate passes succeeded 14/14. A fresh unsigned 2.55 candidate,
installed and portable packaged journeys, the evidence bundle, and publication
remain pending; the latest downloadable release remains 2.54.

An earlier candidate/install attempt is historical: its build was valid, but the
install receipt failed because the wrapper wrongly required byte identity. The
focused-tested wrapper now accepts only Tauri's exact three-byte `UNK` → `NSS`
marker transition and rejects every other byte difference. Fresh candidate,
