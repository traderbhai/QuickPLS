...
Status: final consolidated source gate complete; fresh provenance-bound unsigned candidate build, isolated install, installed and portable smoke, evidence collection and bundle, final audit, merge/tag, and publication gates pending.

Formal first diagnostic `20260822T142953Z` at source `2e3a23f` executed all 14 steps and passed 13; the sole failure was `frontend_typecheck`, where `src/data/v255NamedSemEvidenceFixtures.test.ts` reported TypeScript error `TS2339`. The same-script, same-suite formal final diagnostic `20260823T000930Z` at source `d5b508aa9e8703dfc169a9f224ee76d9a2796d02` passed 14/14, including 453/453 Vitest suites, 1722/1722 tests, 17/17 rebaseline assertions, and zero captured console errors. Its authoritative report is `validation/results/v255_consolidated_diagnostics_20260823T000930Z/v255_consolidated_diagnostics.json` with SHA-256 `F83542DC2BD6427F00583842932B00CB6F551814FBA0BD636F01AB1F0B99C562`; the gate evidence is recorded in commit `ae8f3cea4febf1475f92ea8040bd23cc7e7edff2`. The retained first failure and final pass use runner SHA-256 `64969b4eb89c9789e586c372532d89b082766a44661cc4feea66b6cf3a0f9796`, are separate evidence records, and are not byte-identical. Renderer console/page-error evidence fails closed, attach-only phases use fresh wrapper-owned processes, and candidate/phase/trusted-driver PID, role, suite, and SHA-256 bindings are closed. Version authorities are restored to **2.55.0**. The superseded diagnostics `20260822T202919Z` and `20260822T224933Z`, failed diagnostic attempt `20260822T223906Z`, candidate builds `20260822T205741Z` and `20260822T230754Z`, portable probes `20260822T214629Z` and `20260822T233111Z`, and all other later failed, probe, or build attempts are historical and ineligible. The install comparison permits only the exact Tauri `UNK` → `NSS` three-byte marker transition and rejects every other byte difference. One fresh clean-source provenance-bound unsigned setup, portable, CLI, and checksum package has not yet been built. Isolated installation, full installed-and-portable smoke, final evidence collection and bundling, final audit, and publication remain mandatory. The latest published public pre-release remains `v2.54.0`; code signing is outside this program.
...
- `validation/v255_reusable_archive_inventory.json` version 2 binds the 14 curated static archives by repository path, size, SHA-256, identity, and prior receipt pointer; seven named supplements and the post-hoc route remain explicit candidate-bound lanes.
- `validation/v255_frozen_result_archive_index.json` is the only authority for reusing completed Result evidence. Its version 4 evidence entries must exactly cover every declared Result family and independently bind archive, screenshot, and parsed receipt to a canonical result identity.
...
1. **Complete:** finish T13–T16 product work:
...
2. **Complete:** commit a clean 2.54.0 source-gate baseline with the 64 setup declarations, 18 Calculate-capture declarations, reusable-archive inventory, and fail-closed evidence indexes.
3. **Complete:** run `validation/run_v255_consolidated_diagnostics.ps1` once. Its pre-candidate source gate verifies setup evidence plus the 14 curated static archives and the declared candidate-bound supplement lanes; it does not falsely require a final ZIP before the unsigned candidate exists. It records every failure without stopping at the first ordinary command failure, emits `v255_full_vitest.json`, and requires all 17 rebased records at its final stage. The formal first report is `validation/results/v255_consolidated_diagnostics_20260822T142953Z/v255_consolidated_diagnostics.json`, is bound to source `2e3a23f`, executed all 14 steps, and passed 13; the sole failure was `frontend_typecheck`, where `src/data/v255NamedSemEvidenceFixtures.test.ts` reported TypeScript error `TS2339`.
...
4. **Complete:** correct the focused fixture-test access that caused the first TypeScript `TS2339` failure; close the later strict schema-6 adoption, exact-result readback, metadata-only lineage, and packaged-route defects; then preserve imported factor-variance identities after the `20260822T233111Z` probe exposed orphan duplicates during Advanced equality revision. Failed diagnostic attempt `20260822T223906Z` and all non-formal later attempts are historical and ineligible.
5. **Complete:** run the identical diagnostic script and suite/invocation contract after the final source correction. The formal final report is `validation/results/v255_consolidated_diagnostics_20260823T000930Z/v255_consolidated_diagnostics.json`, is bound to source `d5b508aa9e8703dfc169a9f224ee76d9a2796d02`, and passed 14/14, including 453/453 Vitest suites, 1722/1722 tests, 17/17 rebaseline assertions, and zero captured console errors. Its SHA-256 is `F83542DC2BD6427F00583842932B00CB6F551814FBA0BD636F01AB1F0B99C562`. Both formal diagnostics use runner SHA-256 `64969b4eb89c9789e586c372532d89b082766a44661cc4feea66b6cf3a0f9796`.
6. **Complete:** record the final green gate evidence in commit `ae8f3cea4febf1475f92ea8040bd23cc7e7edff2`, then restore and commit only the approved 2.55 version and release-document authorities. The 2.55 authority commit descends from the final green source-gate evidence and contains no post-gate product-code change.
...
8. If necessary, remove an existing registered QuickPLS installation only through its exact registered uninstaller, verifying that project files, recovery data, and QuickPLS application user data remain untouched and unchanged. Install the fresh report's exact NSIS setup into a new isolated destination with `run_v255_isolated_install.ps1`. Then run `validation/run_v255_installed_portable_smoke.ps1` with the release-artifact report and install receipt, plus the final green consolidated report and its exact Vitest JSON. The smoke derives and rehashes both candidates instead of accepting arbitrary paths. For each candidate it performs a real Calculate → Results → native save → fresh-process reopen journey before its serial breadth crawl. It uses hidden, isolated WebView2/CDP sessions and will only terminate PID trees it created. The portable pass additionally executes the native cross-method routes. The product-owner-approved `-WaiveActualWindows200PercentScaling` switch applies to exactly one named actual-200%-scaling case: that assertion remains false and is recorded as `waived`, while the observed native DPI, browser device-pixel ratio, screenshot, and receipt are still mandatory. The other 54 named cases must pass. Installed and portable journeys are both mandatory; neither substitutes for the other. Then collect current 2.55 reopen captures, the required post-hoc run, and all specialized/cross-method evidence; build the ZIP, update its curated SHA-256 manifest, and rerun the packaged smoke with `-EvidenceBundlePath` for publication verification.
   The portable pass also runs `v255_frozen_archive_reopen_crawler.mjs` serially. It consumes the 14 curated static archives, seven named candidate-bound supplements, and the required post-hoc supplement to produce exactly 18 outer method receipts covering 28 routes, 45 captures, and 71 one-family observations.
...
- The formal first 14-step diagnostic executed every step and passed 13; its sole failure was `frontend_typecheck` with TypeScript error `TS2339` in `src/data/v255NamedSemEvidenceFixtures.test.ts`. The same-script, same-suite formal final diagnostic `20260823T000930Z` passed 14/14. The formal reports are separate and are not byte-identical. Renderer console/page errors fail closed, attach-only journeys have fresh wrapper-owned process boundaries, and candidate/phase/trusted-driver roles and hashes are bound. The superseded diagnostics `20260822T202919Z` and `20260822T224933Z`, failed diagnostic attempt `20260822T223906Z`, candidate builds `20260822T205741Z` and `20260822T230754Z`, portable probes `20260822T214629Z` and `20260822T233111Z`, and all other later failed, probe, or build attempts are historical and ineligible. One fresh unsigned 2.55 candidate build, isolated installation, full installed-and-portable/native-DPI smoke, evidence collection and bundling, final audit, and publication have not yet run.
...
- Any existing registered QuickPLS installation may be removed only through its exact registered uninstaller before the isolated 2.55 install. Project files, recovery data, and QuickPLS application user data must remain untouched and verified unchanged. Installed and portable evidence are both mandatory.
- The release remains unsigned; code signing is outside this program.

## Commands and current handoff

The retained formal first diagnostic is fixed at `validation/results/v255_consolidated_diagnostics_20260822T142953Z/`; it is failure evidence, not a green release authority. The formal final green report and its exact Vitest JSON are fixed under `validation/results/v255_consolidated_diagnostics_20260823T000930Z/`. Build one fresh candidate, then replace the candidate placeholders below with paths from that new report. Do not use the superseded diagnostic `20260822T202919Z`, failed diagnostic attempt `20260822T223906Z`, candidate builds `20260822T205741Z` and `20260822T230754Z`, portable probes `20260822T214629Z` and `20260822T233111Z`, any other later failed, probe, or build attempt, or their hashes.
...
powershell -NoProfile -ExecutionPolicy Bypass -File validation/run_v255_installed_portable_smoke.ps1 `
  ...
  -VitestReportPath validation/results/v255_consolidated_diagnostics_20260823T000930Z/v255_full_vitest.json `
  -ConsolidatedReportPath validation/results/v255_consolidated_diagnostics_20260823T000930Z/v255_consolidated_diagnostics.json `
...
python validation/v255_product_completion_audit.py --publication-stage `
  ...
  --vitest-report validation/results/v255_consolidated_diagnostics_20260823T000930Z/v255_full_vitest.json `
  --rebaseline-report validation/results/v255_consolidated_diagnostics_20260823T000930Z/interaction_contracts/v255_rebased_interaction_contracts.json `
  --first-consolidated-report validation/results/v255_consolidated_diagnostics_20260822T142953Z/v255_consolidated_diagnostics.json `
  --final-consolidated-report validation/results/v255_consolidated_diagnostics_20260823T000930Z/v255_consolidated_diagnostics.json `
