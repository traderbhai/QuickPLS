...
Status: post-hoc-corrected, waiver-qualified consolidated source gate complete; fresh unsigned candidate, installed/portable evidence, bundle, final audit, merge/tag, and publication gates pending.

The post-hoc-corrected formal first and final consolidated passes, `20260822T064330Z` at source `0ae22eb` and `20260822T064832Z` at source `0238e20`, used the same diagnostic script and suite contract and both succeeded 14/14. Version authorities are **2.55.0**. Exact typed post-hoc PLS authority now takes precedence over the generic bootstrap fallback during reopen, and its focused identity suite passed 17/17; that correction has not yet been validated in a packaged candidate. Every earlier candidate, including `20260822T052553Z`, is ineligible because the product correction and current formal validation postdate it. The separate install comparison now permits only the exact Tauri `UNK` → `NSS` three-byte marker transition and rejects every other byte difference. A fresh clean-source provenance-bound setup, portable, CLI, and checksum package has not yet been built. Portable and installed smoke, final evidence collection, and publication remain mandatory; this checkpoint does not claim a downloadable 2.55 release or promote a capability.
...
3. **Complete:** run `validation/run_v255_consolidated_diagnostics.ps1` once. Its pre-candidate source gate verifies setup evidence plus the curated inventory of 17 hashable reusable archives and the one declared later packaged run; it does not falsely require a final ZIP before the unsigned candidate exists. It records every failure without stopping at the first ordinary command failure, emits `v255_full_vitest.json`, and requires all 17 rebased records at its final stage. The post-hoc-corrected formal first report is `validation/results/v255_consolidated_diagnostics_20260822T064330Z/v255_consolidated_diagnostics.json`, is bound to source `0ae22eb`, and passed 14/14.
...
5. **Complete:** run the same diagnostic script and suite contract once more. The final report is `validation/results/v255_consolidated_diagnostics_20260822T064832Z/v255_consolidated_diagnostics.json`, is bound to source `0238e20`, and passed 14/14.
...
- The 14-step consolidated source gate, including its source-level browser crawls, passed in the post-hoc-corrected committed first/final evidence pair. Exact typed post-hoc authority now wins over the generic bootstrap fallback on reopen, and the focused identity suite passed 17/17; packaged validation remains pending. Every earlier candidate, including `20260822T052553Z`, is ineligible because the fix and current source validation came later. The separate exact three-byte Tauri `UNK` → `NSS` marker-only install contract is fixed and focused-tested. A fresh unsigned candidate, installation, installed and portable/native-DPI routes, evidence-bundle collection, and publication have not yet run.
...
The first and final source reports are fixed at `validation/results/v255_consolidated_diagnostics_20260822T064330Z/` and `validation/results/v255_consolidated_diagnostics_20260822T064832Z/`. Build a fresh candidate, then replace the candidate placeholders below with paths from that new report. Do not use any obsolete historical candidate/install attempt, including `20260822T052553Z`, or its hashes.
...
powershell -NoProfile -ExecutionPolicy Bypass -File validation/run_v255_installed_portable_smoke.ps1 `
  ...
  -VitestReportPath validation/results/v255_consolidated_diagnostics_20260822T064832Z/v255_full_vitest.json `
  -ConsolidatedReportPath validation/results/v255_consolidated_diagnostics_20260822T064832Z/v255_consolidated_diagnostics.json `
...
python validation/v255_product_completion_audit.py --publication-stage `
  ...
  --vitest-report validation/results/v255_consolidated_diagnostics_20260822T064832Z/v255_full_vitest.json `
  --rebaseline-report validation/results/v255_consolidated_diagnostics_20260822T064832Z/interaction_contracts/v255_rebased_interaction_contracts.json `
  --first-consolidated-report validation/results/v255_consolidated_diagnostics_20260822T064330Z/v255_consolidated_diagnostics.json `
  --final-consolidated-report validation/results/v255_consolidated_diagnostics_20260822T064832Z/v255_consolidated_diagnostics.json `
