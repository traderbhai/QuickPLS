...
3. **Complete:** run `validation/run_v255_consolidated_diagnostics.ps1` once. Its pre-candidate source gate verifies setup evidence plus the 14 curated static archives and the declared candidate-bound supplement lanes; it does not falsely require a final ZIP before the unsigned candidate exists. It records every failure without stopping at the first ordinary command failure, emits `v255_full_vitest.json`, and requires all 17 rebased records at its final stage. The formal first report is `validation/results/v255_consolidated_diagnostics_20260822T104806Z/v255_consolidated_diagnostics.json`, is bound to source `5ac7d8e`, executed all 14 steps, and passed 13; only `frontend_typecheck` failed.
...
4. **Complete:** apply one remediation batch at source `6289b44`, correcting the typed exact-fit presentation fixture that caused the sole first-run failure.
5. **Complete:** run the identical diagnostic script and suite/invocation contract once more. The final report is `validation/results/v255_consolidated_diagnostics_20260822T105458Z/v255_consolidated_diagnostics.json`, is bound to source `6289b44`, and passed 14/14. Both diagnostics use runner SHA-256 `64969b4eb89c9789e586c372532d89b082766a44661cc4feea66b6cf3a0f9796`.
...
- The first 14-step diagnostic executed every step and passed 13; only `frontend_typecheck` failed. After one remediation batch, the same-script, same-suite final diagnostic passed 14/14. The reports are separate and are not byte-identical. Renderer console/page errors fail closed, attach-only journeys have fresh wrapper-owned process boundaries, and candidate/phase/trusted-driver roles and hashes are bound. Candidate/install attempt `20260822T084527Z` and every earlier attempt are historical and ineligible. A fresh unsigned candidate build, isolated installation, installed and portable/native-DPI routes, evidence collection and bundling, final audit, and publication have not yet run.
...
The retained first diagnostic is fixed at `validation/results/v255_consolidated_diagnostics_20260822T104806Z/`; it is failure evidence, not a green release authority. The final green report and its exact Vitest JSON are fixed under `validation/results/v255_consolidated_diagnostics_20260822T105458Z/`. Build a fresh candidate, then replace the candidate placeholders below with paths from that new report. Do not use candidate/install attempt `20260822T084527Z`, any earlier attempt, or their hashes.
...
powershell -NoProfile -ExecutionPolicy Bypass -File validation/run_v255_installed_portable_smoke.ps1 `
  ...
  -VitestReportPath validation/results/v255_consolidated_diagnostics_20260822T105458Z/v255_full_vitest.json `
  -ConsolidatedReportPath validation/results/v255_consolidated_diagnostics_20260822T105458Z/v255_consolidated_diagnostics.json `
...
python validation/v255_product_completion_audit.py --publication-stage `
  ...
  --vitest-report validation/results/v255_consolidated_diagnostics_20260822T105458Z/v255_full_vitest.json `
  --rebaseline-report validation/results/v255_consolidated_diagnostics_20260822T105458Z/interaction_contracts/v255_rebased_interaction_contracts.json `
  --first-consolidated-report validation/results/v255_consolidated_diagnostics_20260822T104806Z/v255_consolidated_diagnostics.json `
  --final-consolidated-report validation/results/v255_consolidated_diagnostics_20260822T105458Z/v255_consolidated_diagnostics.json `
