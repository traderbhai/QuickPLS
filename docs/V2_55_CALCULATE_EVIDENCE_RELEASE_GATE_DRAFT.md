...
Formal first diagnostic `20260822T125438Z` at source `4bb0810` executed all 14 steps and passed 12; only `v255_evidence_contract` and `v255_final_evidence_contract` failed because README and Installation still reported source 2.55.0 while the gate baseline intentionally remained 2.54.0. Correction commit `2bfcab0` changed only those two documentation version literals. Intermediate diagnostic `20260822T130434Z` passed 14/14 but became ineligible after the root `.gitattributes` checkout-stability source change. The same-script, same-suite formal final diagnostic `20260822T131349Z` at source `245e3e1` passed 14/14, including 453/453 Vitest suites, 1692/1692 tests, all 17 rebaseline assertions, and zero captured console errors. Both formal records use runner SHA-256 `64969b4eb89c9789e586c372532d89b082766a44661cc4feea66b6cf3a0f9796`; they are separate and are not byte-identical. Renderer console/page-error evidence fails closed, attach-only phases use fresh wrapper-owned processes, and candidate/phase/trusted-driver PID, role, suite, and SHA-256 bindings are closed. Version authorities are restored to **2.55.0**. Candidate/install attempt `20260822T110601Z` and every earlier 2.55 candidate are historical and ineligible. The install comparison permits only the exact Tauri `UNK` → `NSS` three-byte marker transition and rejects every other byte difference. A fresh clean-source provenance-bound unsigned setup, portable, CLI, and checksum package has not yet been built. Isolated installation, both installed and portable smoke, final evidence collection and bundling, final audit, and publication remain mandatory. The latest published public pre-release remains `v2.54.0`; code signing is outside this program.
...
3. **Complete:** run `validation/run_v255_consolidated_diagnostics.ps1` once. Its pre-candidate source gate verifies setup evidence plus the 14 curated static archives and the declared candidate-bound supplement lanes; it does not falsely require a final ZIP before the unsigned candidate exists. It records every failure without stopping at the first ordinary command failure, emits `v255_full_vitest.json`, and requires all 17 rebased records at its final stage. The formal first report is `validation/results/v255_consolidated_diagnostics_20260822T125438Z/v255_consolidated_diagnostics.json`, is bound to source `4bb0810`, executed all 14 steps, and passed 12; only `v255_evidence_contract` and `v255_final_evidence_contract` failed because README and Installation still reported source 2.55.0 while the gate baseline intentionally remained 2.54.0.
...
4. **Complete:** apply correction commit `2bfcab0`, changing only the README and Installation source-version literals that caused both first-run contract failures.
5. **Complete:** run the identical diagnostic script and suite/invocation contract after the correction. Intermediate diagnostic `20260822T130434Z` passed 14/14 but became ineligible after the root `.gitattributes` checkout-stability source change. The formal final report is `validation/results/v255_consolidated_diagnostics_20260822T131349Z/v255_consolidated_diagnostics.json`, is bound to source `245e3e1`, and passed 14/14, including 453/453 Vitest suites, 1692/1692 tests, all 17 rebaseline assertions, and zero captured console errors. Both formal diagnostics use runner SHA-256 `64969b4eb89c9789e586c372532d89b082766a44661cc4feea66b6cf3a0f9796`.
6. **Complete:** restore and commit only the approved 2.55 version authorities. The 2.55 authority commit descends from the final green source-gate evidence and contains no post-gate product-code change.
...
- The formal first 14-step diagnostic executed every step and passed 12; only the evidence and final-evidence contracts failed on two stale documentation version literals. Correction commit `2bfcab0` changed README and Installation. Intermediate diagnostic `20260822T130434Z` was green but became ineligible after the root `.gitattributes` checkout-stability source change. The same-script, same-suite formal final diagnostic passed 14/14. The formal reports are separate and are not byte-identical. Renderer console/page errors fail closed, attach-only journeys have fresh wrapper-owned process boundaries, and candidate/phase/trusted-driver roles and hashes are bound. Candidate/install attempt `20260822T110601Z` and every earlier 2.55 candidate are historical and ineligible. A fresh unsigned candidate build, isolated installation, installed and portable/native-DPI routes, evidence collection and bundling, final audit, and publication have not yet run.
...
The retained formal first diagnostic is fixed at `validation/results/v255_consolidated_diagnostics_20260822T125438Z/`; it is failure evidence, not a green release authority. The formal final green report and its exact Vitest JSON are fixed under `validation/results/v255_consolidated_diagnostics_20260822T131349Z/`. Build a fresh candidate, then replace the candidate placeholders below with paths from that new report. Do not use candidate/install attempt `20260822T110601Z`, any earlier 2.55 candidate, or their hashes.
...
powershell -NoProfile -ExecutionPolicy Bypass -File validation/run_v255_installed_portable_smoke.ps1 `
  ...
  -VitestReportPath validation/results/v255_consolidated_diagnostics_20260822T131349Z/v255_full_vitest.json `
  -ConsolidatedReportPath validation/results/v255_consolidated_diagnostics_20260822T131349Z/v255_consolidated_diagnostics.json `
...
python validation/v255_product_completion_audit.py --publication-stage `
  ...
  --vitest-report validation/results/v255_consolidated_diagnostics_20260822T131349Z/v255_full_vitest.json `
  --rebaseline-report validation/results/v255_consolidated_diagnostics_20260822T131349Z/interaction_contracts/v255_rebased_interaction_contracts.json `
  --first-consolidated-report validation/results/v255_consolidated_diagnostics_20260822T125438Z/v255_consolidated_diagnostics.json `
  --final-consolidated-report validation/results/v255_consolidated_diagnostics_20260822T131349Z/v255_consolidated_diagnostics.json `
