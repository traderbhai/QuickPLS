...
Formal first diagnostic `20260822T142953Z` at source `2e3a23f` executed all 14 steps and passed 13; the sole failure was `frontend_typecheck`, where `src/data/v255NamedSemEvidenceFixtures.test.ts` reported TypeScript error `TS2339`. The final consolidated diagnostic `20260823T030939Z` at source `e5723df08b7205ce75f1887c5f4709f235ad893c` passed 14/14, including 453/453 Vitest suites, 1724/1724 tests, 17/17 rebaseline assertions, and zero captured console errors. Its report is `validation/results/v255_consolidated_diagnostics_20260823T030939Z/v255_consolidated_diagnostics.json`, has SHA-256 `03da7a8e0db2924d0157eb0cb0ca92e841fffd61f470d5cd16ccd58f87fe9b2a`, and is retained in evidence commit `8a727262c07dd38bae38d8154e1662c78fbb8ee7`. Both formal records use runner SHA-256 `64969b4eb89c9789e586c372532d89b082766a44661cc4feea66b6cf3a0f9796`; they are separate and are not byte-identical. Renderer console/page-error evidence fails closed, attach-only phases use fresh wrapper-owned processes, and candidate/phase/trusted-driver PID, role, suite, and SHA-256 bindings are closed. Version authorities are restored to **2.55.0**. The superseded final diagnostic `20260823T000930Z`, candidate/install/smoke attempts `20260823T004848Z` and `20260823T005212Z`, portable probe `20260822T233111Z`, and all prior 2.55 candidate, install, smoke, diagnostic, or probe attempts are historical and ineligible. The install comparison permits only the exact Tauri `UNK` → `NSS` three-byte marker transition and rejects every other byte difference. One new clean-source provenance-bound unsigned setup, portable, CLI, and checksum package has not yet been built. Isolated installation, full installed-and-portable smoke, final evidence collection and bundling, final audit, and publication remain mandatory. The latest published public pre-release remains `v2.54.0`; code signing is outside this program.
...
4. **Complete:** correct the focused fixture-test access that caused the sole TypeScript `TS2339` failure.
5. **Complete:** run the identical diagnostic script and suite/invocation contract after the correction. The final consolidated report is `validation/results/v255_consolidated_diagnostics_20260823T030939Z/v255_consolidated_diagnostics.json`, is bound to source `e5723df08b7205ce75f1887c5f4709f235ad893c`, and passed 14/14, including 453/453 Vitest suites, 1724/1724 tests, 17/17 rebaseline assertions, and zero captured console errors. Its SHA-256 is `03da7a8e0db2924d0157eb0cb0ca92e841fffd61f470d5cd16ccd58f87fe9b2a`; both formal diagnostics use runner SHA-256 `64969b4eb89c9789e586c372532d89b082766a44661cc4feea66b6cf3a0f9796`.
6. **Complete:** retain the final green gate evidence in commit `8a727262c07dd38bae38d8154e1662c78fbb8ee7`, then restore only the approved 2.55 version authorities and coordinated documentation. The promotion descends from the report-bound source and contains no post-gate product-code change.
...
- The formal first 14-step diagnostic executed every step and passed 13; its sole failure was `frontend_typecheck` with TypeScript error `TS2339` in `src/data/v255NamedSemEvidenceFixtures.test.ts`. The final consolidated diagnostic passed 14/14. The formal reports are separate and are not byte-identical. Renderer console/page errors fail closed, attach-only journeys have fresh wrapper-owned process boundaries, and candidate/phase/trusted-driver roles and hashes are bound. The superseded final diagnostic `20260823T000930Z`, candidate/install/smoke attempts `20260823T004848Z` and `20260823T005212Z`, portable probe `20260822T233111Z`, and all prior 2.55 candidate, install, smoke, diagnostic, or probe attempts are historical and ineligible. One new unsigned 2.55 candidate build, isolated installation, full installed-and-portable/native-DPI smoke, evidence collection and bundling, final audit, and publication have not yet run.
...
The retained formal first diagnostic is fixed at `validation/results/v255_consolidated_diagnostics_20260822T142953Z/`; it is failure evidence, not a green release authority. The final green report and its exact Vitest JSON are fixed under `validation/results/v255_consolidated_diagnostics_20260823T030939Z/`, bound to source `e5723df08b7205ce75f1887c5f4709f235ad893c`, report SHA-256 `03da7a8e0db2924d0157eb0cb0ca92e841fffd61f470d5cd16ccd58f87fe9b2a`, and evidence commit `8a727262c07dd38bae38d8154e1662c78fbb8ee7`. Build one new candidate, then replace the candidate placeholders below with paths from that new report. Do not use superseded diagnostic `20260823T000930Z`, candidate/install/smoke attempts `20260823T004848Z` or `20260823T005212Z`, portable probe `20260822T233111Z`, any prior 2.55 attempt, or their hashes.
...
powershell -NoProfile -ExecutionPolicy Bypass -File validation/run_v255_installed_portable_smoke.ps1 `
  ...
  -VitestReportPath validation/results/v255_consolidated_diagnostics_20260823T030939Z/v255_full_vitest.json `
  -ConsolidatedReportPath validation/results/v255_consolidated_diagnostics_20260823T030939Z/v255_consolidated_diagnostics.json `
...
python validation/v255_product_completion_audit.py --publication-stage `
  ...
  --vitest-report validation/results/v255_consolidated_diagnostics_20260823T030939Z/v255_full_vitest.json `
  --rebaseline-report validation/results/v255_consolidated_diagnostics_20260823T030939Z/interaction_contracts/v255_rebased_interaction_contracts.json `
  ...
  --final-consolidated-report validation/results/v255_consolidated_diagnostics_20260823T030939Z/v255_consolidated_diagnostics.json `
