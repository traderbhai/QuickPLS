...
- Uses the ML discrepancy between the sample covariance matrix and the optimized model-implied covariance for supported single-group raw-data CFA/SEM models.
...
`npm run qpls:cbsem:fit-reference` checks finite fields, expected bounds, and stable serialized payload shape through `validation/results/cbsem_v07_reference_report.json`. `npm run qpls:cbsem:lavaan-validate` checks supported single-group fit-index parity against lavaan. Focused packaged acceptance verifies the same fit table in native Results, XLSX, the saved typed payload, and the reopened run.
