...
Status: experimental repeated-indicator, two-stage, and hybrid estimator slice.

This document freezes the first QuickPLS recipe contract for higher-order constructs and the initial repeated-indicator, two-stage, and hybrid execution behavior. It does not yet define publication-ready HOC result tables, publication diagrams, exports, published-example evidence, or simulation qualification.
...
Every higher-order declaration emits `method.higher_order.experimental`. Empty HOC placeholder constructs are allowed and emit `higher_order.indicators.generated`, because supported HOC methods can generate the HOC measurement block from lower-order components.
...
The original project recipe remains typed as a HOC declaration. The run emits a warning that repeated-indicator HOCs are experimental and that HOC indicator blocks were expanded from lower-order component indicators.
...
The stage-2 execution keeps lower-order constructs in the model and permits them to be measurement-only components when they have no structural paths in the stage-2 model. The run emits a warning that two-stage HOCs are experimental and that lower-order component scores were used as generated HOC indicators.
...
The original project recipe remains typed as a hybrid HOC declaration. The execution recipe is split before ordinary PLS estimation, and assessment uses the same split so deterministic quality tables assess the same measurement blocks as estimation. Recipe validation emits `higher_order.hybrid_component_indicators` when any hybrid component has fewer than two indicators. Stable exports must continue to treat HOC results as experimental until estimator contracts, published examples, simulation fixtures, metamorphic tests, GUI/CLI parity, result tables, and report outputs pass their gates.

## Acceptance Evidence

The schema is accepted when `cargo test -p qpls-core` proves valid repeated-indicator, two-stage, and hybrid declarations, generated HOC indicator placeholders, duplicate ids, unknown constructs, insufficient components, self-components, unknown components, duplicate components, hybrid component split feasibility, and experimental warnings.

The repeated-indicator estimator slice is accepted when `cargo test -p qpls-estimation` proves that an empty HOC placeholder is expanded from component indicators, produces HOC outer estimates and construct scores, and emits the experimental repeated-indicator warning.

The two-stage estimator slice is accepted when `cargo test -p qpls-estimation` proves that lower-order component scores are appended as generated HOC indicators, the HOC structural path is estimated, and the experimental two-stage HOC warning is emitted. `cargo test -p qpls-assessment` must also pass because assessment has to consume the generated HOC score indicators consistently.

The hybrid estimator slice is accepted when `cargo test -p qpls-estimation` proves that component indicator blocks are split between lower-order and higher-order constructs, the HOC structural path is estimated, and the experimental hybrid HOC warning is emitted. `cargo test -p qpls-assessment` must also pass because assessment has to consume the split HOC recipe consistently.
...
- Sarstedt, Hair, Cheah, Becker, and Ringle (2019) describe HOC specification, estimation, and validation in PLS-SEM using repeated-indicator and two-stage approaches; hybrid remains experimental in QuickPLS until broader evidence is complete.
