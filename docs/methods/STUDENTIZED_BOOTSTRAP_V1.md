...
- Metamorphic tests cover the row-order distributional contract frozen in `RESAMPLING_ENGINE_V4.md`, indicator and construct reordering, admissible scale changes, canonical identities containing delimiters, sign-reversed latent solutions, and native-bound diagnostics. Because a finite seed indexes storage positions rather than stable case identifiers, row-reordered finite Monte Carlo endpoints are compared to a frozen different-seed drift envelope; identical-data repeats and worker-count changes remain exact.
