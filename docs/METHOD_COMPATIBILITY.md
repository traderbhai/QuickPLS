...
| Regression | OLS regression | Validated for documented OLS scope: raw-data OLS with intercept, numeric predictors/controls, complete-case rows, HC3 robust standard errors, t statistics, p values, confidence intervals, fit diagnostics, fitted values, and residuals; logistic, PROCESS, HC0/HC4 claims, categorical encoding, survey weights, clustered SEs, GLS, mixed models, and panel models remain excluded | Yes, documented OLS scope |
| Regression | Logistic and PROCESS | Experimental v0.8 logistic engine plus bounded PROCESS-style mediation/moderation workflow; independent Python fixtures cover current preview scope | No |
