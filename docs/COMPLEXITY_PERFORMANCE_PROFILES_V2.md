...
The current Registry V2 binding contains 43 active catalogue rows and 48 active
option cells. Each cell resolves to 10 profile budgets: five profiles on each
of the Standard and Workstation hardware classes, for 480 resolved budgets.
Because the maximum-axis profile contains four separate cases, a complete
current evidence set contains 768 capability receipts plus eight UI receipts.
