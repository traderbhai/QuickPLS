...
pub enum CompiledPlsInteractionV3Error {
    ...
    #[error(
        "interactions {first_interaction_id} and {second_interaction_id} declare the same operands and focal relation"
    )]
    DuplicateInteractionSignature {
...
pub fn compile_pls_two_way_interactions_v3(
    ...
    let mut signatures = std::collections::BTreeMap::<(String, String, String), String>::new();
    ...
    for term in &model.derived_terms {
        ...
        let signature = (
            operands[0].clone(),
            operands[1].clone(),
            focal_relation.clone(),
        );
        if let Some(first_interaction_id) = signatures.insert(signature, id.clone()) {
            return Err(
                CompiledPlsInteractionV3Error::DuplicateInteractionSignature {
                    first_interaction_id,
                    second_interaction_id: id.clone(),
                },
            );
        }
...
mod tests {
    ...
    fn add_two_way_interaction(
        ...
        model.derived_terms.push(SemDerivedTermV4::InteractionV2 {
            ...
            output: output_id,
            operands: vec!["construct:x".into(), moderator_id.into()],
