...
fn validate_new_identity(
    ...
    for (field, value) in [
        ("source model id", request.source.model_id.as_str()),
        (
            "revision project name",
            request.revision.project_name.as_str(),
        ),
        ("revision model id", request.revision.model_id.as_str()),
        ("revision model name", request.revision.model_name.as_str()),
    ] {
    ...
    for (field, digest) in [
        (
            ...
            request.source.model_document_sha256.as_str(),
        ),
        (
            "source model scientific",
            request.source.model_scientific_sha256.as_str(),
        ),
        (
            "source recipe document",
            request.source.recipe_document_sha256.as_str(),
        ),
    ] {
    ...
    if request.source.project_id.is_nil()
        || request.source.recipe_id.is_nil()
        || request.revision.project_id.is_nil()
        || request.revision.recipe_id.is_nil()
    {
        return Err(GeneralSemExecutionAuthorityRevisionErrorV1::InvalidRequest(
            "project and recipe UUIDs must be non-nil".into(),
        ));
    }
    if request.source.project_id == request.revision.project_id
        || request.source.recipe_id == request.revision.recipe_id
        || request.source.model_id == request.revision.model_id
    {
        return Err(GeneralSemExecutionAuthorityRevisionErrorV1::InvalidRequest(
