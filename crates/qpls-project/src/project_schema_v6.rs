...
const GENERAL_SEM_PLS_TWO_WAY_MODERATED_MEDIATION_BOOTSTRAP_EXECUTION_ADAPTER_VERSION_V1: &str = "compiled_general_sem_pls_recipe_v1_two_way_moderated_mediation_percentile_bootstrap_execution_v1";
const GENERAL_SEM_PLS_SINGLE_MEDIATION_BOOTSTRAP_EXECUTION_ADAPTER_VERSION_V1: &str =
    "compiled_general_sem_pls_recipe_v1_single_mediation_percentile_bootstrap_execution_v1";
const GENERAL_SEM_PLS_THREE_WAY_MODERATION_POINT_EXECUTION_ADAPTER_VERSION_V1: &str =
    "compiled_general_sem_pls_recipe_v1_three_way_moderation_point_execution_v1";
const GENERAL_SEM_PLS_THREE_WAY_MODERATION_BOOTSTRAP_EXECUTION_ADAPTER_VERSION_V1: &str =
    "compiled_general_sem_pls_recipe_v1_three_way_moderation_percentile_bootstrap_execution_v1";
const GENERAL_SEM_PLS_HIGHER_ORDER_POINT_EXECUTION_ADAPTER_VERSION_V1: &str =
    "compiled_general_sem_pls_recipe_v1_disjoint_higher_order_point_execution_v1";
const GENERAL_SEM_PLS_HIGHER_ORDER_BOOTSTRAP_EXECUTION_ADAPTER_VERSION_V1: &str =
    "compiled_general_sem_pls_recipe_v1_disjoint_higher_order_bootstrap_execution_v1";
...
fn validate_general_sem_result_authority_v1(
    ...
    let has_three_way_payload = !results.three_way_interaction_effects.is_empty()
        || !results.three_way_conditional_interaction_effects.is_empty()
        || !results.three_way_simple_slopes.is_empty()
        || results.three_way_moderation_bootstrap_receipt.is_some();
    if has_three_way != has_three_way_payload {
        return Err(invalid_general_sem_authority(
            "the canonical three-way payload does not match the resident compiled model",
        ));
    }
    let (expected_method_version, expected_engine_version) = if has_higher_order {
        if has_interactions {
            return Err(invalid_general_sem_authority(
                "the bounded HOC authority cannot contain interactions",
            ));
        }
        ...
        match resident_config.inference {
            GeneralSemInferenceV1::None => (
                GENERAL_SEM_PLS_THREE_WAY_MODERATION_POINT_METHOD_VERSION_V1,
                GENERAL_SEM_PLS_THREE_WAY_MODERATION_POINT_EXECUTION_ADAPTER_VERSION_V1,
            ),
            GeneralSemInferenceV1::CaseBootstrap { .. } => (
                GENERAL_SEM_PLS_THREE_WAY_MODERATION_BOOTSTRAP_METHOD_VERSION_V1,
                GENERAL_SEM_PLS_THREE_WAY_MODERATION_BOOTSTRAP_EXECUTION_ADAPTER_VERSION_V1,
            ),
        }
    } else if has_interactions {
        match (is_moderated_mediation, resident_config.inference) {
        ...
        match resident_config.inference {
            ...
            GeneralSemInferenceV1::CaseBootstrap { .. } if is_single_mediation => (
                GENERAL_SEM_PLS_SINGLE_MEDIATION_CASE_BOOTSTRAP_METHOD_VERSION_V1,
                GENERAL_SEM_PLS_SINGLE_MEDIATION_BOOTSTRAP_EXECUTION_ADAPTER_VERSION_V1,
            ),
            GeneralSemInferenceV1::CaseBootstrap { .. } => (
    ...
    if has_three_way {
        return validate_general_sem_pls_three_way_authority_v1(
            document,
            results,
            &artifact,
            resident_config,
...
fn validate_general_sem_pls_three_way_authority_v1(
    document: &CanonicalResultDocumentV2,
    results: &qpls_core::CanonicalGeneralSemResultsV1,
    artifact: &qpls_core::CompiledGeneralSemPlsRecipeV1,
    config: &qpls_core::GeneralSemConfigV1,
    recipe: &AnalysisRecipeV4,
) -> Result<(), ProjectArchiveV6Error> {
    let compiled = artifact.plan().three_way_interaction().ok_or_else(|| {
        invalid_general_sem_authority(
            "the three-way canonical result has no resident compiled three-way interaction",
        )
    })?;
    let point_cell = qpls_core::pls_general_three_way_moderation_point_capability_cell_v1();
    let bootstrap_cell = qpls_core::pls_general_three_way_moderation_bootstrap_capability_cell_v1();
    let inferred = matches!(
        config.inference,
        GeneralSemInferenceV1::CaseBootstrap { .. }
    );

    if artifact.capability_cell() != &point_cell
        || artifact.supplemental_capability_admission().is_some()
        || results.inference_receipt.is_some()
        || results.three_way_moderation_bootstrap_receipt.is_some() != inferred
        || results.three_way_interaction_effects.len() != 1
        || results.three_way_conditional_interaction_effects.is_empty()
        || results.three_way_simple_slopes.is_empty()
        || results.interaction_effects.len() != artifact.plan().two_way_interactions().len()
        || results.joint_stage_structural_coefficients.is_empty()
        || !results.specific_indirect_effects.is_empty()
        || !results.aggregate_effects.is_empty()
        || !results.conditional_indirect_effects.is_empty()
    ...
    {
        return Err(invalid_general_sem_authority(
            "the bounded three-way result has a missing, foreign, or incorrectly scoped typed result section",
        ));
    }

    let effect = &results.three_way_interaction_effects[0];
    if effect.interaction_id != compiled.interaction_id()
        || effect.focal_relation_id != compiled.focal_relation_id()
        || effect.interaction_effect_relation_id != compiled.interaction_effect_relation_id()
        || effect.interaction_effect_parameter_id != compiled.interaction_effect_parameter_id()
        || effect.operand_ids.as_slice() != compiled.operand_ids().as_slice()
        || effect.outcome_id != compiled.outcome_id()
        || effect.generated_product_column_id != compiled.generated_product_column_id()
        || effect.stage_one_model_scientific_sha256
            != artifact
                .plan()
                .stage_one_projection_scientific_sha256()
                .unwrap_or_default()
        || effect.trace.capability_cell != point_cell
    {
        return Err(invalid_general_sem_authority(
            "the canonical three-way coefficient differs from the resident compiled interaction",
        ));
    }
    if results
        ...
        .iter()
        .any(|row| {
            row.interaction_id != compiled.interaction_id()
                || row.focal_relation_id != compiled.focal_relation_id()
                || row.first_moderator_id != compiled.first_moderator_id()
                || row.second_moderator_id != compiled.second_moderator_id()
                || row.trace.capability_cell != point_cell
        })
        || results.three_way_simple_slopes.iter().any(|row| {
            row.interaction_id != compiled.interaction_id()
                || row.focal_relation_id != compiled.focal_relation_id()
                || row.first_moderator_id != compiled.first_moderator_id()
                || row.second_moderator_id != compiled.second_moderator_id()
    ...
    {
        return Err(invalid_general_sem_authority(
            "the canonical three-way probe grid differs from the resident compiled interaction",
        ));
    }

    let persisted_lower_order_ids = results
        .interaction_effects
        .iter()
        .map(|row| row.interaction_id.as_str())
        .collect::<BTreeSet<_>>();
    let compiled_lower_order_ids = artifact
        .plan()
        .two_way_interactions()
        .iter()
        .map(|row| row.interaction_id())
        .collect::<BTreeSet<_>>();
    if persisted_lower_order_ids != compiled_lower_order_ids {
        return Err(invalid_general_sem_authority(
            "the canonical lower-order hierarchy differs from the resident three-way plan",
        ));
    }

    let base = project_capability_cell_v2(
        &RecipeV4CompilerTarget::PlsPlanV2.capability_cell_for_method(recipe.settings.method),
    );
    let point = project_capability_cell_v2(&point_cell);
    let bootstrap = project_capability_cell_v2(&bootstrap_cell);
    let mut expected_document_cells = vec![base, point.clone()];
    if inferred {
    ...
    let table_cells = if inferred {
        vec![point.clone(), bootstrap.clone()]
    } else {
        vec![point.clone()]
    };
    let expected_table_ids = if inferred {
        vec![
            ...
            GENERAL_SEM_THREE_WAY_RECEIPT_TABLE_ID_V1,
        ]
    } else {
        vec![
            GENERAL_SEM_THREE_WAY_EFFECT_TABLE_ID_V1,
    ...
    let sections = document
        ...
        .collect::<Vec<_>>();
    let Some(section) = sections.first().copied().filter(|_| sections.len() == 1) else {
        return Err(invalid_general_sem_authority(
            "the exact three-way result section must occur once",
        ));
    };
    if document.capability_cells.as_deref() != Some(expected_document_cells.as_slice())
        ...
        || document.presentation.default_section_id.as_deref()
            != Some(GENERAL_SEM_THREE_WAY_SECTION_ID_V1)
        || document.presentation.default_table_id.as_deref()
            != Some(GENERAL_SEM_THREE_WAY_SIMPLE_SLOPES_TABLE_ID_V1)
        || section
            .table_ids
            .iter()
            .map(String::as_str)
            .ne(expected_table_ids.iter().copied())
        || section.capability_cells.as_deref() != Some(table_cells.as_slice())
    {
        return Err(invalid_general_sem_authority(
            "the exact three-way document, section, or presentation authority has drifted",
        ));
    }
    for (index, table_id) in expected_table_ids.iter().enumerate() {
        let tables = document
            .tables
            .iter()
            .filter(|table| table.id == *table_id)
            .collect::<Vec<_>>();
        let Some(table) = tables.first().copied().filter(|_| tables.len() == 1) else {
            return Err(invalid_general_sem_authority(format!(
                "the exact three-way table {table_id} must occur once"
            )));
        };
        let expected_cells = if inferred && index + 1 == expected_table_ids.len() {
            std::slice::from_ref(&bootstrap)
        } else {
            table_cells.as_slice()
        };
        if table.capability_cells.as_deref() != Some(expected_cells) {
            return Err(invalid_general_sem_authority(format!(
                "the exact three-way table {table_id} has a foreign capability owner"
            )));
        }
    }
    if document.exclusions.len() != 1
        || document.exclusions[0].id != "three_way_v1_bounded_scope"
        || document.exclusions[0].capability_cell.as_ref()
            != Some(if inferred { &bootstrap } else { &point })
    {
        return Err(invalid_general_sem_authority(
            "the bounded three-way scope declaration has drifted",
        ));
    }
    ...
    match (
        ...
        (GeneralSemInferenceV1::None, None) => Ok(()),
        (
            GeneralSemInferenceV1::CaseBootstrap {
                resamples,
                seed,
            ...
            if receipt.capability_cell != bootstrap_cell
                || receipt.capability_dependencies != expected_dependencies
                || receipt.compiled_plan_sha256 != artifact.plan().deterministic_sha256()
                || receipt.general_sem_config_sha256 != artifact.general_sem_config_sha256()
                || receipt.model_scientific_sha256 != artifact.plan().scientific_hash()
                || receipt.stage_one_model_scientific_sha256
                    != artifact
                        .plan()
                        .stage_one_projection_scientific_sha256()
                        .unwrap_or_default()
                || receipt.source_dataset_fingerprint != document.provenance.dataset_fingerprint
            ...
            {
                return Err(invalid_general_sem_authority(
                    "the three-way bootstrap receipt differs from the resident plan, config, dataset, or scheduler request",
                ));
            }
            Ok(())
        }
        _ => Err(invalid_general_sem_authority(
            "three-way v1 requires point-only output or percentile two-sided full-model case-bootstrap inference",
        )),
    }
}
...
const GENERAL_SEM_HIGHER_ORDER_POINT_STAGE_RECEIPT_VERSION_V1: &str =
    "general_sem_pls_higher_order_point_stage_receipt_v1";
const GENERAL_SEM_DISJOINT_HOC_SCORE_DATASET_RECEIPT_VERSION_V1: &str =
    "general_sem_pls_disjoint_hoc_score_dataset_receipt_v1";
const GENERAL_SEM_DISJOINT_HOC_BOOTSTRAP_OPERATION_VERSION_V1: &str =
    "general_sem_pls_higher_order_full_model_case_bootstrap_operation_v1";
const GENERAL_SEM_DISJOINT_HOC_SIGN_ALIGNMENT_VERSION_V1: &str =
    "sampled_original_construct_score_covariance_v1";
const GENERAL_SEM_DISJOINT_HOC_TARGET_VERSION_V1: &str =
    "compiled_hoc_component_and_structural_relation_target_v1";
const GENERAL_SEM_HIGHER_ORDER_STAGE_COLUMNS_V1: &[&str] = &[
    "stage_id",
    "higher_order_construct_id",
    "stage_number",
    "stage_kind",
    "approach",
    "measurement_type",
    ...
    "generated_score_receipt_version",
    "complete_case_rows_sha256",
];
const GENERAL_SEM_HIGHER_ORDER_TARGET_COLUMNS_V1: &[&str] = &[
    "relation_id",
    "parameter_id",
    "kind",
    "source_id",
...
const GENERAL_SEM_HIGHER_ORDER_BOOTSTRAP_RECEIPT_COLUMNS_V1: &[&str] = &[
    "capability_id",
    "cell_id",
    "capability_version",
    "method_version",
    ...
    "stage_one_reestimated_per_replicate",
    "generated_component_values_recalculated_per_replicate",
    "stage_one_scores_sign_aligned_per_replicate",
    "stage_two_reestimated_per_replicate",
    "stage_two_scores_sign_aligned_per_replicate",
    "complete_point_contract_validated_per_replicate",
    "failed_replicate_count",
];

fn validate_general_sem_pls_higher_order_authority_v1(
    document: &CanonicalResultDocumentV2,
    results: &qpls_core::CanonicalGeneralSemResultsV1,
    artifact: &qpls_core::CompiledGeneralSemPlsRecipeV1,
    recipe: &AnalysisRecipeV4,
    ...
    let point =
        project_capability_cell_v2(&qpls_core::pls_general_higher_order_point_capability_cell_v1());
    let bootstrap = project_capability_cell_v2(
        &qpls_core::pls_general_higher_order_bootstrap_capability_cell_v1(),
    );
    let base = project_capability_cell_v2(
        &RecipeV4CompilerTarget::PlsPlanV2.capability_cell_for_method(recipe.settings.method),
    );
    let is_bootstrap = results.higher_order_inference_receipt.is_some();
    let mut expected_cells = vec![base, point.clone()];
    if is_bootstrap {
        expected_cells.push(bootstrap.clone());
    }
    ...
    if document.capability_cells.as_deref() != Some(expected_cells.as_slice())
        ...
        || document.presentation.default_table_id.as_deref()
            != Some(GENERAL_SEM_HIGHER_ORDER_TARGETS_TABLE_ID_V1)
    {
        return Err(invalid_general_sem_authority(
            "the HOC document capability set, title, or presentation authority has drifted",
        ));
    }
    let expected_sections = if is_bootstrap {
        vec![
            GENERAL_SEM_HIGHER_ORDER_SECTION_ID_V1,
            GENERAL_SEM_HIGHER_ORDER_BOOTSTRAP_SECTION_ID_V1,
        ]
    } else {
        vec![GENERAL_SEM_HIGHER_ORDER_SECTION_ID_V1]
    };
    if document
        .sections
        .iter()
        .rev()
        .take(expected_sections.len())
        .map(|section| section.id.as_str())
        .ne(expected_sections.iter().rev().copied())
    {
        return Err(invalid_general_sem_authority(
            "the exact HOC section order has drifted",
        ));
    }
    let point_section = document
        .sections
        .iter()
        .find(|section| section.id == GENERAL_SEM_HIGHER_ORDER_SECTION_ID_V1)
        .ok_or_else(|| invalid_general_sem_authority("the HOC point section is absent"))?;
    if point_section.table_ids
        != [
            GENERAL_SEM_HIGHER_ORDER_STAGES_TABLE_ID_V1,
            GENERAL_SEM_HIGHER_ORDER_TARGETS_TABLE_ID_V1,
        ]
        || point_section.capability_cells.as_deref() != Some(std::slice::from_ref(&point))
    {
        return Err(invalid_general_sem_authority(
            "the HOC point section tables or capability ownership have drifted",
        ));
    }
    if is_bootstrap {
        let section = document
            .sections
            .iter()
            .find(|section| section.id == GENERAL_SEM_HIGHER_ORDER_BOOTSTRAP_SECTION_ID_V1)
            .ok_or_else(|| invalid_general_sem_authority("the HOC bootstrap section is absent"))?;
        if section.table_ids != [GENERAL_SEM_HIGHER_ORDER_BOOTSTRAP_RECEIPT_TABLE_ID_V1]
            || section.capability_cells.as_deref() != Some(std::slice::from_ref(&bootstrap))
        {
            return Err(invalid_general_sem_authority(
                "the HOC bootstrap section table or capability ownership has drifted",
            ));
        }
    }

    if results.higher_order_stages.len() != hoc.stage_projections().len() {
        return Err(invalid_general_sem_authority(
    ...
    for (stage, projection) in results.higher_order_stages.iter().zip(ordered_projections) {
        ...
        if stage.stage_id
            ...
            || stage.higher_order_construct_id != hoc.output_variable_id()
            || stage.stage_number != u32::from(projection.stage_number())
            || stage.kind != expected_kind
            || stage.approach.as_ref() != Some(hoc.approach())
            || stage.measurement_type.as_ref() != Some(hoc.measurement_type())
            || stage.trace.model_id != document.provenance.model_id
            || stage.trace.capability_cell
                != qpls_core::pls_general_higher_order_point_capability_cell_v1()
            || receipt.receipt_version != GENERAL_SEM_HIGHER_ORDER_POINT_STAGE_RECEIPT_VERSION_V1
    ...
    let expected_mappings = hoc
        ...
        .iter()
        .map(
            |mapping| qpls_core::CanonicalHocGeneratedVariableMappingV1 {
                component_id: mapping.component_id().to_string(),
                generated_score_variable_id: mapping.generated_score_variable_id().to_string(),
                generated_component_relation_id: mapping
                    .generated_component_relation_id()
                    .to_string(),
                generated_component_parameter_id: mapping
                    .generated_component_parameter_id()
                    .to_string(),
                component_relation_source_id: mapping.component_relation_source_id().to_string(),
                component_relation_target_id: mapping.component_relation_target_id().to_string(),
                relation_interpretation: mapping.relation_interpretation(),
            },
        )
        .collect::<Vec<_>>();
    ...
    for mapping in hoc.component_mappings() {
        let relation = persisted_relations
            .get(mapping.generated_component_relation_id())
            .ok_or_else(|| {
                invalid_general_sem_authority(format!(
                    "compiled HOC component {} has no canonical target row",
                    mapping.component_id()
                ))
            })?;
        let expected_kind = match mapping.relation_interpretation() {
            qpls_core::CompiledPlsHocComponentRelationInterpretationV1::Loading => {
                qpls_core::CanonicalHocRelationKindV1::ComponentLoading
            }
            qpls_core::CompiledPlsHocComponentRelationInterpretationV1::WeightAndCollinearity => {
                qpls_core::CanonicalHocRelationKindV1::ComponentWeight
            }
        };
        if relation.parameter_id.as_deref() != Some(mapping.generated_component_parameter_id())
            || relation.source_id != mapping.component_relation_source_id()
            || relation.target_id != mapping.component_relation_target_id()
            || relation.kind != Some(expected_kind)
        {
            return Err(invalid_general_sem_authority(
                "a canonical HOC component relation differs from its compiled mapping",
            ));
        }
    }
    for compiled in artifact.plan().topology().structural_relations() {
        let relation = persisted_relations
            .get(compiled.relation_id())
            .ok_or_else(|| invalid_general_sem_authority("an authored HOC relation is absent"))?;
        let expected_kind = match compiled.role() {
            StructuralRelationRoleV4::Structural => {
                qpls_core::CanonicalHocRelationKindV1::AuthoredStructural
            }
            StructuralRelationRoleV4::Control => {
                qpls_core::CanonicalHocRelationKindV1::AuthoredControl
            }
        };
        if relation.parameter_id.as_deref() != Some(compiled.parameter_id())
            || relation.source_id != compiled.source()
            || relation.target_id != compiled.target()
            || relation.kind != Some(expected_kind)
        {
            return Err(invalid_general_sem_authority(
                "an authored canonical HOC relation differs from the compiled topology",
            ));
        }
    }
    ...
    let expected_exclusion = if is_bootstrap {
        "higher_order_bootstrap_bounded_targets_only"
    } else {
        "higher_order_point_estimation_only"
    };
    let expected_exclusion_cell = if is_bootstrap { &bootstrap } else { &point };
    if !matches!(document.exclusions.as_slice(), [exclusion]
        if exclusion.id == expected_exclusion
            && exclusion.capability_cell.as_ref() == Some(expected_exclusion_cell)
            && !exclusion.title.trim().is_empty()
            && !exclusion.reason.trim().is_empty())
    {
        return Err(invalid_general_sem_authority(
            "the HOC exclusion identity or capability ownership has drifted",
        ));
    }
    Ok(())
}

fn validate_higher_order_bootstrap_authority_v1(
    results: &qpls_core::CanonicalGeneralSemResultsV1,
    artifact: &qpls_core::CompiledGeneralSemPlsRecipeV1,
    recipe: &AnalysisRecipeV4,
    dataset: &DatasetDescriptor,
    ...
    let config = recipe.general_sem_config.as_ref().ok_or_else(|| {
        invalid_general_sem_authority("the HOC result has no resident GeneralSemConfigV1")
    })?;
    match (&results.higher_order_inference_receipt, config.inference) {
        (None, GeneralSemInferenceV1::None) => {
            if results.higher_order_stages.iter().any(|stage| {
                stage
                    .relation_estimates
                    .iter()
                    .any(|relation| general_sem_estimate_has_inference_v1(&relation.value))
            }) {
                return Err(invalid_general_sem_authority(
                    "the point-only HOC result contains inference fields",
                ));
            }
            Ok(())
        }
        (
            ...
            let mut expected_identities = hoc
                .component_mappings()
                .iter()
                .map(|mapping| qpls_core::CanonicalHocBootstrapTargetIdentityV1 {
                    kind: match mapping.relation_interpretation() {
                        qpls_core::CompiledPlsHocComponentRelationInterpretationV1::Loading => {
                            qpls_core::CanonicalHocBootstrapTargetKindV1::ComponentLoading
                        }
                        qpls_core::CompiledPlsHocComponentRelationInterpretationV1::WeightAndCollinearity => {
                            qpls_core::CanonicalHocBootstrapTargetKindV1::ComponentWeight
                        }
                    },
                    target_version: "compiled_hoc_component_and_structural_relation_target_v1".into(),
                ...
                .chain(
                    artifact
                        ...
                        .map(|relation| qpls_core::CanonicalHocBootstrapTargetIdentityV1 {
                            kind: qpls_core::CanonicalHocBootstrapTargetKindV1::HocStructuralPath,
                            target_version: "compiled_hoc_component_and_structural_relation_target_v1".into(),
            ...
            if receipt.capability_cell
                != qpls_core::pls_general_higher_order_bootstrap_capability_cell_v1()
                || project_capability_cell_v2(&receipt.capability_cell) != bootstrap_cell.clone()
                || receipt.method_version
                    != qpls_core::PLS_GENERAL_HIGHER_ORDER_BOOTSTRAP_CAPABILITY_VERSION_V1
                || receipt.point_method_version
                    != qpls_core::PLS_GENERAL_HIGHER_ORDER_POINT_CAPABILITY_VERSION_V1
                || receipt.resampling_operation_version
                    != GENERAL_SEM_DISJOINT_HOC_BOOTSTRAP_OPERATION_VERSION_V1
                ...
                || receipt.sign_alignment_method_version
                    != GENERAL_SEM_DISJOINT_HOC_SIGN_ALIGNMENT_VERSION_V1
                || receipt.target_version != GENERAL_SEM_DISJOINT_HOC_TARGET_VERSION_V1
                || receipt.general_sem_config_sha256 != artifact.general_sem_config_sha256()
                || receipt.compiled_plan_sha256 != artifact.plan().deterministic_sha256()
                || receipt.hoc_stage_plan_sha256 != qpls_core::sha256_serialized(hoc)
                || receipt.model_scientific_sha256 != artifact.plan().scientific_hash()
                || receipt.stage_one_model_scientific_sha256
                    != artifact.plan().base_plan().scientific_hash()
                ...
                || receipt.target_identity_set_sha256
                    != qpls_core::sha256_serialized(&expected_identities)
                || receipt.resamples_requested != resamples
                || receipt.seed != seed.to_string()
                || receipt.confidence_level.to_bits() != confidence_level.to_bits()
                || receipt.interval
                    != qpls_core::CanonicalGeneralSemBootstrapIntervalV1::PercentileType7
                || receipt.tail != qpls_core::CanonicalGeneralSemInferenceTailV1::TwoSided
                || usize::try_from(receipt.workers).ok() != Some(recipe.settings.workers)
                || recipe.settings.bootstrap_samples != resamples
                || recipe.settings.seed != seed
                || recipe.settings.confidence_level.to_bits() != confidence_level.to_bits()
            {
                return Err(invalid_general_sem_authority(
                    "the HOC bootstrap receipt differs from the resident plan, target inventory, or inference settings",
                ));
            }
            Ok(())
        }
        _ => Err(invalid_general_sem_authority(
            "the resident HOC inference config and typed bootstrap receipt disagree",
        )),
    }
}

fn general_sem_estimate_has_inference_v1(value: &qpls_core::CanonicalGeneralSemEstimateV1) -> bool {
...
fn validate_higher_order_tables_v1(
    document: &CanonicalResultDocumentV2,
    results: &qpls_core::CanonicalGeneralSemResultsV1,
    point_cell: &crate::CapabilityCellReferenceV2,
    bootstrap_cell: &crate::CapabilityCellReferenceV2,
    is_bootstrap: bool,
) -> Result<(), ProjectArchiveV6Error> {
    let table = hoc_table_v1(document, GENERAL_SEM_HIGHER_ORDER_STAGES_TABLE_ID_V1)?;
    validate_hoc_columns_v1(table, GENERAL_SEM_HIGHER_ORDER_STAGE_COLUMNS_V1)?;
    if table.capability_cells.as_deref() != Some(std::slice::from_ref(point_cell))
        || table.rows.len() != results.higher_order_stages.len()
    {
        return Err(invalid_general_sem_authority(
            "the HOC stage table ownership or row inventory has drifted",
        ));
    }
    for (index, (row, stage)) in table
        .rows
        .iter()
        .zip(results.higher_order_stages.iter())
        .enumerate()
    {
        let receipt = stage.receipt.as_ref().ok_or_else(|| {
            invalid_general_sem_authority("the HOC stage table lacks a typed receipt")
        })?;
        let generated = receipt.generated_score_dataset.as_ref();
        let input_ids = stage.input_construct_ids.join(";");
        let output_ids = stage.output_variable_ids.join(";");
        if row.id != format!("higher_order_stage_{index:04}") || row.cells.len() != 18 {
            return Err(invalid_general_sem_authority(
                "the HOC stage table row identity or width has drifted",
            ));
        }
        for (cell, expected, context) in [
            (&row.cells[0], stage.stage_id.as_str(), "stage id"),
            (
                &row.cells[1],
                stage.higher_order_construct_id.as_str(),
                "HOC id",
            ),
            (
                &row.cells[3],
                hoc_stage_kind_label_v1(stage.kind),
                "stage kind",
            ),
            (
                &row.cells[4],
                stage
                    .approach
                    .as_ref()
                    .map_or("missing", hoc_approach_label_v1),
                "HOC approach",
            ),
            (
                &row.cells[5],
                stage
                    .measurement_type
                    .as_ref()
                    .map_or("missing", hoc_measurement_type_label_v1),
                "HCM type",
            ),
            (
                &row.cells[6],
                hoc_stage_role_label_v1(receipt.role),
                "stage role",
            ),
            (&row.cells[7], input_ids.as_str(), "stage inputs"),
            (&row.cells[8], output_ids.as_str(), "stage outputs"),
            (
                &row.cells[9],
        ...
        moderation_number_cell_v1(&row.cells[2], f64::from(stage.stage_number), "stage number")?;
        moderation_number_cell_v1(
            &row.cells[14],
            f64::from(receipt.used_observations),
            "used rows",
        )?;
        moderation_number_cell_v1(
            &row.cells[15],
            f64::from(receipt.omitted_observations),
            "omitted rows",
        )?;
        moderation_optional_text_cell_v1(
            &row.cells[16],
            generated.map(|value| value.receipt_version.as_str()),
            "score receipt version",
        )?;
        moderation_optional_text_cell_v1(
            &row.cells[17],
            generated.map(|value| value.complete_case_rows_sha256.as_str()),
            "complete-case row digest",
        )?;
    }
        ...
        .enumerate()
    {
        if row.id != format!("higher_order_target_{index:04}") || row.cells.len() != 15 {
            return Err(invalid_general_sem_authority(
                "the HOC target table row identity or width has drifted",
            ));
        }
        moderation_text_cell_v1(&row.cells[0], &relation.relation_id, "target relation")?;
        moderation_optional_text_cell_v1(
            &row.cells[1],
            relation.parameter_id.as_deref(),
            "target parameter",
        )?;
        moderation_optional_text_cell_v1(
            &row.cells[2],
            relation.kind.map(hoc_relation_kind_label_v1),
            "target kind",
        )?;
        moderation_text_cell_v1(&row.cells[3], &relation.source_id, "target source")?;
        moderation_text_cell_v1(&row.cells[4], &relation.target_id, "target outcome")?;
        moderation_number_cell_v1(&row.cells[5], relation.value.estimate, "target estimate")?;
        for (cell, expected, context) in [
            (&row.cells[6], relation.collinearity_vif, "target VIF"),
            (
                &row.cells[7],
                relation.value.bootstrap_mean,
                "bootstrap mean",
            ),
            (
                &row.cells[8],
                relation.value.bootstrap_bias,
                "bootstrap bias",
            ),
            (&row.cells[9], relation.value.standard_error, "bootstrap SE"),
            (&row.cells[10], relation.value.lower, "bootstrap lower"),
            (&row.cells[11], relation.value.upper, "bootstrap upper"),
            (&row.cells[12], relation.value.p_value, "bootstrap p"),
            (
                &row.cells[13],
                relation.value.bootstrap_usable_replicates.map(f64::from),
                "usable replicates",
            ),
            (
                &row.cells[14],
            ...
            moderation_optional_number_cell_v1(cell, expected, context)?;
        }
    }
    if is_bootstrap {
        let receipt = results
            ...
            .as_ref()
            .ok_or_else(|| {
                invalid_general_sem_authority("the HOC bootstrap table has no typed receipt")
            })?;
        let table = hoc_table_v1(
            document,
            GENERAL_SEM_HIGHER_ORDER_BOOTSTRAP_RECEIPT_TABLE_ID_V1,
        )?;
        validate_hoc_columns_v1(table, GENERAL_SEM_HIGHER_ORDER_BOOTSTRAP_RECEIPT_COLUMNS_V1)?;
        if table.capability_cells.as_deref() != Some(std::slice::from_ref(bootstrap_cell))
            || !matches!(table.rows.as_slice(), [row] if row.id == "higher_order_bootstrap_receipt" && row.cells.len() == 39)
        {
            return Err(invalid_general_sem_authority(
                "the HOC bootstrap receipt table ownership or row inventory has drifted",
            ));
        }
        let row = &table.rows[0];
        let joined_target_ids = receipt.target_ids.join(";");
        for (cell, expected, context) in [
            (
                &row.cells[0],
            ...
            (&row.cells[3], receipt.method_version.as_str(), "method"),
            (
                &row.cells[4],
                receipt.point_method_version.as_str(),
                "point method",
            ),
            (
                &row.cells[5],
            ...
            (
                &row.cells[12],
                receipt.sign_alignment_method_version.as_str(),
                "sign alignment",
            ),
            (
                &row.cells[13],
            ...
            (&row.cells[24], joined_target_ids.as_str(), "target ids"),
            (&row.cells[29], receipt.seed.as_str(), "seed"),
        ] {
            moderation_text_cell_v1(cell, expected, context)?;
        }
        for (cell, expected, context) in [
            (&row.cells[25], receipt.confidence_level, "confidence"),
            (
                &row.cells[26],
            ...
            (&row.cells[30], f64::from(receipt.workers), "workers"),
            (
                &row.cells[38],
        ...
        for (cell, expected, context) in [
            ...
            hoc_boolean_cell_v1(cell, expected, context)?;
        }
    } else if document
        .tables
        .iter()
        .any(|table| table.id == GENERAL_SEM_HIGHER_ORDER_BOOTSTRAP_RECEIPT_TABLE_ID_V1)
    {
        return Err(invalid_general_sem_authority(
            "the point-only HOC document contains a bootstrap receipt table",
        ));
    }
    Ok(())
}
...
fn hoc_table_v1<'a>(
    ...
    document
        ...
        .ok_or_else(|| invalid_general_sem_authority(format!("HOC table {table_id} is absent")))
}

fn hoc_stage_kind_label_v1(kind: qpls_core::CanonicalHocStageKindV1) -> &'static str {
    match kind {
        qpls_core::CanonicalHocStageKindV1::LowerOrderScoreEstimation => {
            "lower_order_score_estimation"
        }
        qpls_core::CanonicalHocStageKindV1::HigherOrderEstimation => "higher_order_estimation",
    }
}
...
fn hoc_measurement_type_label_v1(
    measurement_type: &qpls_core::HigherOrderMeasurementTypeV4,
) -> &'static str {
    match measurement_type {
        qpls_core::HigherOrderMeasurementTypeV4::ReflectiveReflective => "reflective_reflective",
        qpls_core::HigherOrderMeasurementTypeV4::ReflectiveFormative => "reflective_formative",
        qpls_core::HigherOrderMeasurementTypeV4::FormativeReflective => "formative_reflective",
        qpls_core::HigherOrderMeasurementTypeV4::FormativeFormative => "formative_formative",
    }
}

fn hoc_stage_role_label_v1(role: qpls_core::CompiledPlsHocStageRoleV1) -> &'static str {
    match role {
        qpls_core::CompiledPlsHocStageRoleV1::RepeatedIndicatorEstimation => {
...
fn hoc_relation_kind_label_v1(kind: qpls_core::CanonicalHocRelationKindV1) -> &'static str {
    match kind {
        qpls_core::CanonicalHocRelationKindV1::ComponentLoading => "component_loading",
...
fn validate_hoc_columns_v1(
    ...
    {
        return Err(invalid_general_sem_authority(format!(
            "HOC table {} column contract has drifted",
            table.id
        )));
    }
    Ok(())
...
fn moderation_optional_text_cell_v1(
    cell: &CanonicalResultCellV2,
    expected: Option<&str>,
    context: &str,
) -> Result<(), ProjectArchiveV6Error> {
    match (cell, expected) {
        (CanonicalResultCellV2::Text { value }, Some(expected)) if value == expected => Ok(()),
        (
        ...
        _ => Err(invalid_general_sem_authority(format!(
            "{context} differs from the typed HOC payload"
        ))),
    }
}

fn hoc_boolean_cell_v1(
    cell: &CanonicalResultCellV2,
    expected: bool,
    context: &str,
) -> Result<(), ProjectArchiveV6Error> {
...
fn validate_general_sem_pls_interaction_authority_v1(
    ...
    if artifact
        ...
        .is_some()
    {
        validate_general_sem_pls_moderated_mediation_document_v1(
            document, results, artifact, recipe,
        )
    } else {
...
const GENERAL_SEM_MODERATED_MEDIATION_GAMMA_COLUMNS_V1: &[&str] = &[
    "effect_id",
    "interaction_id",
    "focal_relation_id",
    "interaction_effect_relation_id",
...
const GENERAL_SEM_CONDITIONAL_INDIRECT_COLUMNS_V1: &[&str] = &[
    "effect_id",
    "target_id",
    "estimand_id",
    "moderated_stage",
...
const GENERAL_SEM_MODERATED_MEDIATION_INDEX_COLUMNS_V1: &[&str] = &[
    "effect_id",
    "target_id",
    "estimand_id",
    "moderated_stage",
...
const GENERAL_SEM_MODERATED_MEDIATION_RECEIPT_COLUMNS_V1: &[&str] = &[
    "capability_id",
    "cell_id",
    "capability_version",
    "base_capability_id",
...
fn moderated_mediation_stage_label_v1(
...
fn validate_moderated_mediation_inference_cells_v1(
    row: &crate::CanonicalResultRowV2,
    offset: usize,
    estimate: &qpls_core::CanonicalGeneralSemEstimateV1,
    context: &str,
) -> Result<(), ProjectArchiveV6Error> {
    let values = [
    ...
    if values.iter().any(Option::is_none) || row.cells.len() < offset + values.len() {
        return Err(invalid_general_sem_authority(format!(
            "{context} does not contain complete five-target inference"
        )));
    }
    for (index, expected) in values.into_iter().enumerate() {
        moderation_number_cell_v1(
            &row.cells[offset + index],
            expected.unwrap(),
            &format!("{context} inference cell {index}"),
        )?;
    }
    Ok(())
}

fn moderated_mediation_raw_identity_digest_v1(
    gamma: &qpls_core::CanonicalInteractionEffectResultV1,
    target: &qpls_core::CompiledPlsTwoWayModeratedMediationTargetV1,
) -> String {
    #[derive(Serialize)]
    struct FiveTargetIdentityV1<'a> {
        gamma_target: &'a qpls_resampling::GeneralSemPlsModerationGammaTargetIdentityV1,
        derived_targets: &'a [qpls_core::GeneralSemPlsTwoWayModeratedMediationInferenceTargetV1],
    }
    let gamma_target = qpls_resampling::GeneralSemPlsModerationGammaTargetIdentityV1 {
        kind: qpls_resampling::GeneralSemPlsModerationGammaTargetKindV1::InteractionScientificRescaledGamma,
        target_version:
            qpls_resampling::GENERAL_SEM_PLS_MULTIPLE_MODERATION_GAMMA_TARGET_VERSION_V1.into(),
        target_id: gamma.effect_id.clone(),
        interaction_id: gamma.interaction_id.clone(),
        focal_relation_id: gamma.focal_relation_id.clone(),
        interaction_effect_relation_id: gamma.interaction_effect_relation_id.clone(),
        interaction_effect_parameter_id: gamma.interaction_effect_parameter_id.clone(),
        generated_product_column_id: gamma.generated_product_column_id.clone(),
        focal_predictor_id: gamma.focal_predictor_id.clone(),
        moderator_id: gamma.moderator_id.clone(),
        outcome_id: gamma.outcome_id.clone(),
        stage_one_model_scientific_sha256: gamma.stage_one_model_scientific_sha256.clone(),
        product_scale_version: gamma.product_scale_version.clone(),
        method_version: gamma.method_version.clone(),
    };
    let mut derived_targets =
        qpls_core::general_sem_pls_two_way_moderated_mediation_inference_targets_v1(target);
    derived_targets.sort_by(|left, right| {
        moderated_mediation_target_effect_id_v1(left)
            .cmp(moderated_mediation_target_effect_id_v1(right))
    });
    sha256_serialized(&FiveTargetIdentityV1 {
        gamma_target: &gamma_target,
        derived_targets: &derived_targets,
    })
}

fn moderated_mediation_target_effect_id_v1(
...
fn validate_general_sem_pls_moderated_mediation_document_v1(
    document: &CanonicalResultDocumentV2,
    results: &qpls_core::CanonicalGeneralSemResultsV1,
    artifact: &qpls_core::CompiledGeneralSemPlsRecipeV1,
    recipe: &AnalysisRecipeV4,
    ...
    let target = artifact
        ...
        .ok_or_else(|| {
            invalid_general_sem_authority(
                "moderated-mediation persistence requires an exact recompiled target",
            )
        })?;
    let receipt = results.inference_receipt.as_ref().ok_or_else(|| {
        invalid_general_sem_authority(
            "moderated-mediation persistence requires its combined bootstrap receipt",
        )
    })?;
    let base = moderation_base_capability_cell_v1(recipe);
    let point = project_capability_cell_v2(artifact.capability_cell());
    let supplemental = project_capability_cell_v2(target.bootstrap_capability_cell());
    let mut expected_dependencies = vec![
        target.base_pls_capability_cell().clone(),
        target.moderation_point_capability_cell().clone(),
    ];
    expected_dependencies.sort_by_key(qpls_core::capability_cell_reference_identity_v2);
    if &receipt.capability_cell != target.bootstrap_capability_cell()
        || receipt.capability_dependencies != expected_dependencies
        || receipt.method_version
            != GENERAL_SEM_PLS_TWO_WAY_MODERATED_MEDIATION_BOOTSTRAP_METHOD_VERSION_V1
        || receipt.compilation_artifact_identity_sha256 != artifact.artifact_identity_sha256()
        || receipt.compiled_plan_sha256 != artifact.plan().deterministic_sha256()
        || receipt.general_sem_config_sha256 != artifact.general_sem_config_sha256()
        || receipt.recipe_analytical_sha256 != artifact.recipe_analytical_sha256()
        || !receipt.complete_model_reestimated_per_replicate
    ...
    {
        return Err(invalid_general_sem_authority(
            "the moderated-mediation document capability set, title, or presentation target has drifted",
        ));
    }
    ...
    {
        return Err(invalid_general_sem_authority(
            "the moderated-mediation typed inventory exceeds or omits the exact five-target scope",
        ));
    }
    validate_general_sem_moderation_inference_surface_v1(results, true)?;

    let moderation_section = document
        .sections
        .iter()
        .find(|section| section.id == GENERAL_SEM_MODERATION_SECTION_ID_V1)
        .ok_or_else(|| invalid_general_sem_authority("the moderation point section is absent"))?;
    let combined_section = document
        .sections
        .iter()
        .find(|section| section.id == GENERAL_SEM_MODERATED_MEDIATION_BOOTSTRAP_SECTION_ID_V1)
        .ok_or_else(|| {
            invalid_general_sem_authority("the moderated-mediation inference section is absent")
        })?;
    ...
    if document.sections.last().map(|section| section.id.as_str())
        ...
        || combined_section
            ...
            .iter()
            .map(String::as_str)
            .ne(expected_combined_tables)
        || moderation_section.capability_cells.as_deref() != Some(std::slice::from_ref(&point))
        || combined_section.capability_cells.as_deref() != Some(std::slice::from_ref(&supplemental))
        || !combined_section.chart_ids.is_empty()
    {
        return Err(invalid_general_sem_authority(
            "the moderated-mediation section membership, order, or ownership has drifted",
        ));
    }
    let table_suffix = document
        ...
        .iter()
        .rev()
        .take(expected_combined_tables.len())
        .map(|table| table.id.as_str())
        .collect::<Vec<_>>();
    if table_suffix
    ...
    {
        return Err(invalid_general_sem_authority(
            "the moderated-mediation inference table order has drifted",
        ));
    }
    for (table_id, columns, expected_rows) in [
        (
            GENERAL_SEM_MODERATED_MEDIATION_GAMMA_TABLE_ID_V1,
            GENERAL_SEM_MODERATED_MEDIATION_GAMMA_COLUMNS_V1,
            1usize,
        ),
        (
            GENERAL_SEM_CONDITIONAL_INDIRECT_TABLE_ID_V1,
            GENERAL_SEM_CONDITIONAL_INDIRECT_COLUMNS_V1,
            3,
        ),
        (
            GENERAL_SEM_MODERATED_MEDIATION_INDEX_TABLE_ID_V1,
            GENERAL_SEM_MODERATED_MEDIATION_INDEX_COLUMNS_V1,
            1,
        ),
        (
            GENERAL_SEM_MODERATED_MEDIATION_RECEIPT_TABLE_ID_V1,
            GENERAL_SEM_MODERATED_MEDIATION_RECEIPT_COLUMNS_V1,
            1,
        ),
    ] {
        ...
        let [table] = matches.as_slice() else {
            return Err(invalid_general_sem_authority(format!(
                "the moderated-mediation document requires exactly one {table_id} table"
            )));
        };
        validate_moderation_table_columns_v1(table, columns)?;
        if table.rows.len() != expected_rows
            || table.capability_cells.as_deref() != Some(std::slice::from_ref(&supplemental))
        {
            return Err(invalid_general_sem_authority(format!(
                "moderated-mediation table {table_id} has drifted row inventory or ownership"
            )));
        }
    }
    let expected_exclusions = [
        "moderated_mediation_single_two_relation_target_only",
        "moderated_mediation_locked_standardized_probes",
        "moderated_mediation_no_causal_claim",
    ];
    if document
        .exclusions
        .iter()
        .map(|exclusion| exclusion.id.as_str())
        .ne(expected_exclusions)
        || document
            .exclusions
            .iter()
            .any(|exclusion| exclusion.capability_cell.as_ref() != Some(&supplemental))
    {
        return Err(invalid_general_sem_authority(
            "the moderated-mediation exclusion inventory or ownership has drifted",
        ));
    }

    validate_general_sem_joint_stage_structural_ledger_v1(document, results, artifact)?;
    validate_general_sem_moderation_result_tables_v1(document, results, &point)?;
    validate_general_sem_moderation_charts_v1(document, results)?;

    let gamma = &results.interaction_effects[0];
    if gamma.interaction_id != target.interaction_id()
        || gamma.focal_relation_id != target.moderated_relation_id()
        || gamma.interaction_effect_relation_id != target.interaction_effect_relation_id()
        || gamma.interaction_effect_parameter_id != target.interaction_effect_parameter_id()
        || gamma.generated_product_column_id != target.generated_product_column_id()
        || gamma.moderator_id != target.moderator_id()
        || gamma.stage_one_model_scientific_sha256 != target.stage_one_model_scientific_sha256()
    {
        return Err(invalid_general_sem_authority(
            "the scientific gamma differs from the recompiled conditional-process target",
        ));
    }
    let expected_stage = match target.moderated_stage() {
        qpls_core::CompiledPlsTwoWayModeratedMediationStageV1::FirstStage => {
            qpls_core::CanonicalModeratedMediationStageV1::FirstStage
        }
        qpls_core::CompiledPlsTwoWayModeratedMediationStageV1::SecondStage => {
            qpls_core::CanonicalModeratedMediationStageV1::SecondStage
        }
    };
    for effect in results
        ...
        .map(|effect| {
            (
                effect.target_id.as_str(),
                effect.estimand_id.as_str(),
                effect.moderated_stage,
                effect.interaction_id.as_str(),
                effect.x_id.as_str(),
                effect.mediator_id.as_str(),
                effect.y_id.as_str(),
                effect.moderator_id.as_str(),
                effect.ordered_relation_ids.as_slice(),
            )
        })
    ...
    {
        if effect.0 != target.target_id()
            || effect.1 != target.estimand_id()
            || effect.2 != expected_stage
            || effect.3 != target.interaction_id()
            || effect.4 != target.x_id()
            || effect.5 != target.mediator_id()
            || effect.6 != target.y_id()
            || effect.7 != target.moderator_id()
            || effect.8 != target.ordered_relation_ids()
        {
            return Err(invalid_general_sem_authority(
                "a conditional indirect or index row differs from the recompiled target",
            ));
        }
    }

    let gamma_table =
        moderation_table(document, GENERAL_SEM_MODERATED_MEDIATION_GAMMA_TABLE_ID_V1)?;
    let gamma_row = &gamma_table.rows[0];
    for (cell, expected, context) in [
        (
            &gamma_row.cells[0],
            gamma.effect_id.as_str(),
            "gamma effect",
        ),
        (
            &gamma_row.cells[1],
            gamma.interaction_id.as_str(),
            "gamma interaction",
        ),
        (
            &gamma_row.cells[2],
            gamma.focal_relation_id.as_str(),
            "gamma focal relation",
        ),
        (
            &gamma_row.cells[3],
            gamma.interaction_effect_relation_id.as_str(),
            "gamma effect relation",
        ),
        (
            &gamma_row.cells[4],
            gamma.interaction_effect_parameter_id.as_str(),
            "gamma parameter",
        ),
        (
            &gamma_row.cells[5],
            gamma.generated_product_column_id.as_str(),
            "gamma product",
        ),
        (
            &gamma_row.cells[6],
            gamma.focal_predictor_id.as_str(),
            "gamma focal predictor",
        ),
        (
            &gamma_row.cells[7],
            gamma.moderator_id.as_str(),
            "gamma moderator",
        ),
        (
            &gamma_row.cells[8],
            gamma.outcome_id.as_str(),
            "gamma outcome",
        ),
        (
            &gamma_row.cells[9],
            gamma.stage_one_model_scientific_sha256.as_str(),
            "gamma stage-one digest",
        ),
        (
            &gamma_row.cells[10],
            gamma.product_scale_version.as_str(),
            "gamma product scale",
        ),
        (
            &gamma_row.cells[11],
            gamma.method_version.as_str(),
            "gamma point method",
        ),
    ] {
        moderation_text_cell_v1(cell, expected, context)?;
    }
    validate_moderated_mediation_inference_cells_v1(
        gamma_row,
        12,
        &gamma.scientific_rescaled_gamma,
        "moderated-mediation gamma row",
    )?;

    let conditional_table =
        moderation_table(document, GENERAL_SEM_CONDITIONAL_INDIRECT_TABLE_ID_V1)?;
    for (index, (row, effect)) in conditional_table
        .rows
        .iter()
        .zip(&results.conditional_indirect_effects)
        .enumerate()
    {
        if row.id != format!("conditional_indirect_{index:04}") {
            return Err(invalid_general_sem_authority(
                "a conditional-indirect row identity has drifted",
            ));
        }
        let ordered = serde_json::to_string(&effect.ordered_relation_ids)
            .map_err(|error| invalid_general_sem_authority(error.to_string()))?;
        for (cell, expected, context) in [
            (
                &row.cells[0],
            ...
            (
                &row.cells[3],
                moderated_mediation_stage_label_v1(effect.moderated_stage),
                "conditional stage",
            ),
            (
                &row.cells[4],
            ...
            (&row.cells[5], effect.x_id.as_str(), "conditional X"),
            (&row.cells[6], effect.mediator_id.as_str(), "conditional M"),
            (&row.cells[7], effect.y_id.as_str(), "conditional Y"),
            (&row.cells[8], effect.moderator_id.as_str(), "conditional W"),
            (&row.cells[9], ordered.as_str(), "conditional relations"),
        ] {
            moderation_text_cell_v1(cell, expected, context)?;
        }
        moderation_number_cell_v1(
        ...
        validate_moderated_mediation_inference_cells_v1(
            ...
            12,
            &effect.value,
            "conditional-indirect row",
        )?;
    }

    let index_effect = &results.moderated_mediation_indices[0];
    let index_table =
        moderation_table(document, GENERAL_SEM_MODERATED_MEDIATION_INDEX_TABLE_ID_V1)?;
    let index_row = &index_table.rows[0];
    let index_relations = serde_json::to_string(&index_effect.ordered_relation_ids)
        .map_err(|error| invalid_general_sem_authority(error.to_string()))?;
    for (cell, expected, context) in [
        (
            &index_row.cells[0],
            index_effect.effect_id.as_str(),
            "index effect",
        ),
        (
            &index_row.cells[1],
            index_effect.target_id.as_str(),
            "index target",
        ),
        (
            &index_row.cells[2],
            index_effect.estimand_id.as_str(),
            "index estimand",
        ),
        (
            &index_row.cells[3],
            moderated_mediation_stage_label_v1(index_effect.moderated_stage),
            "index stage",
        ),
        (
            &index_row.cells[4],
            index_effect.interaction_id.as_str(),
            "index interaction",
        ),
        (&index_row.cells[5], index_effect.x_id.as_str(), "index X"),
        (
            &index_row.cells[6],
            index_effect.mediator_id.as_str(),
            "index M",
        ),
        (&index_row.cells[7], index_effect.y_id.as_str(), "index Y"),
        (
            &index_row.cells[8],
            index_effect.moderator_id.as_str(),
            "index W",
        ),
        (
            &index_row.cells[9],
            index_relations.as_str(),
            "index relations",
        ),
    ] {
        moderation_text_cell_v1(cell, expected, context)?;
    ...
    let receipt_row = &receipt_table.rows[0];
    let five_target_ids = serde_json::to_string(&receipt.effect_ids)
        .map_err(|error| invalid_general_sem_authority(error.to_string()))?;
    let compiled_target_sha256 = target.deterministic_sha256();
    let raw_identity_sha256 = moderated_mediation_raw_identity_digest_v1(gamma, target);
    let expected_text = [
        (0, receipt.capability_cell.capability_id.as_str()),
        (1, receipt.capability_cell.cell_id.as_str()),
        (2, receipt.capability_cell.capability_version.as_str()),
        (3, target.base_pls_capability_cell().capability_id.as_str()),
        (4, target.base_pls_capability_cell().cell_id.as_str()),
        (
            5,
            target
                .base_pls_capability_cell()
                .capability_version
                .as_str(),
        ),
        (
        ...
        (
            7,
            target.moderation_point_capability_cell().cell_id.as_str(),
        ),
        (
            8,
            target
        ...
        (9, receipt.method_version.as_str()),
        (10, GENERAL_SEM_PLS_MULTIPLE_TWO_WAY_POINT_METHOD_VERSION_V1),
        (11, receipt.resampling_operation_version.as_str()),
        (12, receipt.resampling_stream_version.as_str()),
        (13, receipt.quantile_method_version.as_str()),
        (14, receipt.standard_error_method_version.as_str()),
        (15, receipt.summation_method_version.as_str()),
        (16, receipt.p_value_method_version.as_str()),
        (17, receipt.failure_policy_version.as_str()),
        (
            18,
            qpls_resampling::GENERAL_SEM_PLS_MULTIPLE_MODERATION_SIGN_ALIGNMENT_VERSION_V1,
        ),
        (19, qpls_core::GENERAL_SEM_PLS_PRODUCT_SCALE_VERSION_V1),
        (
            20,
            qpls_resampling::GENERAL_SEM_PLS_MULTIPLE_MODERATION_GAMMA_TARGET_VERSION_V1,
        ),
        (21, target.contract_version()),
        (22, target.target_id()),
        (23, compiled_target_sha256.as_str()),
        (24, target.probe_policy_version()),
        (25, target.conditional_target_version()),
        (26, target.index_target_version()),
        (27, receipt.compilation_artifact_identity_sha256.as_str()),
        (28, receipt.compiled_plan_sha256.as_str()),
        (29, receipt.general_sem_config_sha256.as_str()),
        (30, receipt.recipe_analytical_sha256.as_str()),
        (31, receipt.model_scientific_sha256.as_str()),
        (32, target.stage_one_model_scientific_sha256()),
        (33, receipt.source_dataset_fingerprint.as_str()),
        (34, receipt.complete_case_frame_sha256.as_str()),
        (35, receipt.usable_replicate_indices_sha256.as_str()),
        (36, raw_identity_sha256.as_str()),
        (37, receipt.effect_identity_set_sha256.as_str()),
        (38, five_target_ids.as_str()),
        (39, "percentile_type7"),
        (40, "two_sided"),
        (45, receipt.seed.as_str()),
    ];
    for (index, expected) in expected_text {
        moderation_text_cell_v1(
            &receipt_row.cells[index],
            expected,
            &format!("moderated-mediation receipt cell {index}"),
        )?;
    }
    for (index, expected) in [
        (41, receipt.confidence_level),
        (42, f64::from(receipt.resamples_requested)),
        (43, f64::from(receipt.resamples_usable)),
        (44, f64::from(receipt.minimum_usable_resamples)),
        (46, f64::from(receipt.workers)),
        (54, receipt.failed_replicates.len() as f64),
    ] {
        moderation_number_cell_v1(
            &receipt_row.cells[index],
            expected,
...
mod tests {
    ...
    -> (ProjectArchiveDocumentV6, CanonicalResultDocumentV2) {
        let dataset = qpls_data::import_delimited_bytes(
            b"x1,x2,m1,m2,y1,y2\n1,2,2,1,2,1\n2,1,3,2,3,2\n3,4,4,3,5,4\n4,3,5,5,6,5\n5,6,7,6,8,7\n6,5,6,7,9,8\n7,8,9,7,11,9\n8,7,8,9,10,11\n9,10,11,10,13,12\n10,9,12,11,14,13\n11,12,13,12,16,15\n12,11,14,13,17,16\n",
            "general-sem-schema6-single-mediation.csv",
            b',',
            &qpls_data::ImportOptions::default(),
        )
        .unwrap();
        let source_model = ModelSpec {
            id: Uuid::from_u128(0x6a11),
            name: "General SEM single-mediation authority".into(),
            constructs: ["x", "m", "y"]
                .into_iter()
                .map(|id| Construct {
                    id: id.into(),
        ...
        *dataset_id = dataset.id.to_string();
        ...
        assert_eq!(
            ...
            Some(&single_mediation_cell)
        );

        let mut project = ProjectArchiveDocumentV6::new_general_sem_v1(
            Uuid::from_u128(0x6a13),
            "General SEM single-mediation authority fixture",
            Utc.timestamp_opt(1_800_000_000, 0).unwrap(),
        );
        project.datasets.push(DatasetDescriptor::from(&dataset));
        project.models.push(ProjectModelRecordV6 {
        ...
        crate::write_project_data_lineage_v1(
            ...
            &crate::ProjectDataLineageV1 {
                ...
                records: vec![crate::ProjectDatasetVersionRecordV1 {
                    ...
                    operation: crate::ProjectDatasetVersionOperationV1::Import,
                    created_at: None,
                    summary: "Imported schema-6 single-mediation authority fixture".into(),
                    source_column: None,
                    target_column: None,
                    transformation: None,
                }],
        ...
        let mut capability_cells = vec![
            project_capability_cell_v2(&single_mediation_cell),
            project_capability_cell_v2(artifact.capability_cell()),
        ];
        sort_project_capability_cells_v1(&mut capability_cells);
        let canonical = CanonicalResultDocumentV2 {
            schema_version: 2,
            document_id: "general_sem_single_mediation_authority_document".into(),
            title: "General SEM PLS single-mediation bootstrap results".into(),
            provenance: CanonicalResultProvenanceV2 {
                run_id: "general_sem_single_mediation_authority_run".into(),
                project_id: project.project_id.to_string(),
                model_id: model.id,
                model_digest: model_scientific_sha256,
                dataset_id: dataset.id.to_string(),
                ...
                recipe_digest: artifact.recipe_analytical_sha256().into(),
                capability_cell: project_capability_cell_v2(artifact.capability_cell()),
                method_version: GENERAL_SEM_PLS_SINGLE_MEDIATION_CASE_BOOTSTRAP_METHOD_VERSION_V1
                    .into(),
                engine_version: execution.adapter_version().into(),
                seed: Some(recipe.settings.seed),
                workers: recipe.settings.workers as u32,
    ...
    fn add_schema6_three_way_interaction(
        model: &mut SemModelV4,
        interaction_id: &str,
        operand_ids: [&str; 3],
    ) {
        let focal_relation = model
            ...
            .find_map(|relation| match relation {
                SemRelationV4::Structural {
                    ...
                    role: StructuralRelationRoleV4::Structural,
                    ..
                } if source == operand_ids[0] && target == "construct:y" => Some(id.clone()),
                _ => None,
            })
            .expect("the three-way fixture has the requested focal path");
        let output = format!("derived:{interaction_id}");
        let effect_relation = format!("relation:{interaction_id}:effect");
        let effect_parameter = format!("parameter:{interaction_id}:effect");
        model.variables.push(SemVariableV4::Derived {
        ...
        model.derived_terms.push(SemDerivedTermV4::InteractionV2 {
            id: interaction_id.to_string(),
            output,
            operands: operand_ids.into_iter().map(str::to_string).collect(),
            focal_relation,
            method: InteractionMethodV4::TwoStage,
            hierarchy_policy: InteractionHierarchyPolicyV2::Strong,
            product_indicator: None,
    ...
    fn general_sem_schema6_three_way_authority_fixture()
    -> (ProjectArchiveDocumentV6, CanonicalResultDocumentV2) {
        let source_model = ModelSpec {
            id: Uuid::from_u128(0x4d4f_4452_3341_5941_3601),
            name: "Schema-6 three-way moderation authority".into(),
            constructs: ["x", "w", "z", "y"]
                .into_iter()
                .map(|id| Construct {
                    id: id.into(),
        ...
        for row in 0..81 {
            ...
            let w = (row * 0.71).sin() + 0.2 * (row * 0.13).cos();
            let z = (row * 0.37).cos() - 0.3 * (row * 0.19).sin();
            let y = 0.25 * x + 0.20 * w - 0.15 * z + 0.35 * x * w + 0.30 * x * w * z;
            csv.push_str(&format!(
                "{},{},{},{},{},{},{},{}\n",
                x + 0.03 * (row * 0.11).sin(),
                1.02 * x + 0.02 * (row * 0.17).cos(),
        ...
        let dataset = qpls_data::import_delimited_bytes(
            csv.as_bytes(),
            "schema6-three-way-moderation.csv",
            b',',
            &qpls_data::ImportOptions::default(),
        )
        .unwrap();
        let source_recipe = AnalysisRecipe {
            schema_version: ANALYSIS_RECIPE_SCHEMA_VERSION,
            id: Uuid::from_u128(0x4d4f_4452_3341_5941_3602),
            created_at: Utc.timestamp_opt(1_700_000_000, 0).unwrap(),
            dataset_fingerprint: dataset.fingerprint.0.clone(),
            model: source_model.clone(),
            settings: AnalysisSettings {
        ...
        *dataset_id = dataset.id.to_string();
        ...
        add_schema6_three_way_interaction(
            &mut model,
        ...
        let mut project = ProjectArchiveDocumentV6::new_general_sem_v1(
            Uuid::from_u128(0x4d4f_4452_3341_5941_3603),
            "Schema-6 three-way moderation authority",
            Utc.timestamp_opt(1_800_000_000, 0).unwrap(),
        );
        project.datasets.push(DatasetDescriptor::from(&dataset));
        project.models.push(ProjectModelRecordV6 {
        ...
        crate::write_project_data_lineage_v1(
            ...
            &crate::ProjectDataLineageV1 {
                ...
                records: vec![crate::ProjectDatasetVersionRecordV1 {
                    ...
                    operation: crate::ProjectDatasetVersionOperationV1::Import,
                    created_at: None,
                    summary: "Imported schema-6 three-way moderation authority fixture".into(),
                    source_column: None,
                    target_column: None,
                    transformation: None,
                }],
        ...
        let point_cell = project_capability_cell_v2(artifact.capability_cell());
        let mut capability_cells = vec![
            moderation_base_capability_cell_v1(&recipe),
            point_cell.clone(),
        ];
        sort_project_capability_cells_v1(&mut capability_cells);
        let table_ids = [
        ...
        let tables = table_ids
            ...
            .map(|id| {
                nonlinear_test_table(
                    ...
                    vec![CanonicalResultRowV2 {
                        id: format!("{id}_row"),
                        cells: vec![nonlinear_test_text("persisted")],
                    }],
                    point_cell.clone(),
                )
            })
        ...
        let canonical = CanonicalResultDocumentV2 {
            schema_version: 2,
            document_id: "general_sem_three_way_authority_document".into(),
            title: "General SEM three-way PLS moderation point estimates".into(),
            provenance: CanonicalResultProvenanceV2 {
                run_id: "general_sem_three_way_authority_run".into(),
                project_id: project.project_id.to_string(),
                model_id: model.id,
                model_digest: model_scientific_sha256,
                dataset_id: dataset.id.to_string(),
                ...
                recipe_id: recipe.id.to_string(),
                recipe_digest: artifact.recipe_analytical_sha256().into(),
                capability_cell: point_cell.clone(),
            ...
            capability_cells: Some(capability_cells),
            general_sem_results: Some(general_sem_results),
            sections: vec![CanonicalResultSectionV2 {
                id: GENERAL_SEM_THREE_WAY_SECTION_ID_V1.into(),
                title: "Three-way moderation".into(),
                description: Some("Typed three-way moderation fixture".into()),
                table_ids: table_ids.into_iter().map(str::to_string).collect(),
                chart_ids: Vec::new(),
            ...
            charts: Vec::new(),
            notices: Vec::new(),
            exclusions: vec![crate::CanonicalResultExclusionV2 {
                id: "three_way_v1_bounded_scope".into(),
                capability_cell: Some(point_cell),
                title: "Bounded three-way scope".into(),
                reason: "One qualified three-way term per model".into(),
            }],
            footnotes: Vec::new(),
            presentation: CanonicalResultPresentationV2 {
                default_section_id: Some(GENERAL_SEM_THREE_WAY_SECTION_ID_V1.into()),
                default_table_id: Some(GENERAL_SEM_THREE_WAY_SIMPLE_SLOPES_TABLE_ID_V1.into()),
                precision: 4,
                missing_value_label: "—".into(),
                chart_defaults: CanonicalChartDisplayOptionsV2::default(),
            },
    ...
    fn general_sem_result_is_bound_to_resident_schema6_authorities() {
        ...
        assert_eq!(
            GENERAL_SEM_PLS_TWO_WAY_MODERATED_MEDIATION_BOOTSTRAP_EXECUTION_ADAPTER_VERSION_V1,
            qpls_runner::RECIPE_V4_GENERAL_SEM_PLS_TWO_WAY_MODERATED_MEDIATION_BOOTSTRAP_EXECUTION_ADAPTER_VERSION_V1
        );
        assert_eq!(
            GENERAL_SEM_PLS_HIGHER_ORDER_POINT_EXECUTION_ADAPTER_VERSION_V1,
            qpls_runner::RECIPE_V4_GENERAL_SEM_PLS_HIGHER_ORDER_POINT_EXECUTION_ADAPTER_VERSION_V1
        );
        assert_eq!(
            GENERAL_SEM_PLS_HIGHER_ORDER_BOOTSTRAP_EXECUTION_ADAPTER_VERSION_V1,
            qpls_runner::RECIPE_V4_GENERAL_SEM_PLS_HIGHER_ORDER_BOOTSTRAP_EXECUTION_ADAPTER_VERSION_V1
        );
    ...
    #[test]
    fn single_mediation_exact_cell_reopens_with_stable_identity() {
        let (project, canonical) = general_sem_schema6_single_mediation_authority_fixture();
        project.ensure_valid().unwrap();
        let expected_results = canonical.general_sem_results.as_ref().unwrap();
        assert_eq!(expected_results.specific_indirect_effects.len(), 1);
        let expected_effect_id = expected_results.specific_indirect_effects[0]
            .effect_id
            .clone();
        let expected_path = expected_results.specific_indirect_effects[0]
            .ordered_relation_ids
            .clone();
        let expected_attachment_sha256 = project.canonical_result_documents[0]
            .canonical_document_sha256()
            .to_string();

        let bytes = serialize_project_document_v6(&project).unwrap();
        let reopened = deserialize_project_document_v6(&bytes).unwrap();
        reopened.ensure_valid().unwrap();
        let attachment = &reopened.canonical_result_documents[0];
        let document = attachment.canonical_document();
        ...
        assert_eq!(results.specific_indirect_effects.len(), 1);
        assert_eq!(
            results.specific_indirect_effects[0].effect_id,
            expected_effect_id
        );
        assert_eq!(
        ...
        assert_eq!(
            attachment.canonical_document_sha256(),
            expected_attachment_sha256.as_str()
        );
    }

    #[test]
    fn three_way_canonical_append_reopen_and_tamper_fail_closed() {
        let (mut source, canonical) = general_sem_schema6_three_way_authority_fixture();
        let expected_effect = canonical
            .general_sem_results
            .as_ref()
            .unwrap()
            .three_way_interaction_effects[0]
            .clone();
        source.canonical_result_documents.clear();
        source.ensure_valid().unwrap();

        let directory = tempfile::tempdir().unwrap();
        let destination = directory.path().join("three-way-canonical-v6.json");
        write_project_document_v6_new(&destination, &source).unwrap();
        let before_sha256 = sha256_file_v6(&destination).unwrap();
        let receipt =
        ...
        assert_eq!(
            results.three_way_interaction_effects[0].effect_id,
            expected_effect.effect_id
        );
        assert_eq!(
            results.three_way_interaction_effects[0].operand_ids,
            expected_effect.operand_ids
        );

        let mut tampered = serde_json::to_value(&reopened).unwrap();
        tampered["canonical_result_documents"][0]["canonical_document"]["general_sem_results"]["three_way_interaction_effects"]
            [0]["scientific_rescaled_delta"]["estimate"] = Value::from(99.0);
        assert!(matches!(
            deserialize_project_document_v6(&serde_json::to_vec(&tampered).unwrap()),
            Err(ProjectArchiveV6Error::CanonicalResultDocument(_))
        ));
    }
